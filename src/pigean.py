...
class GeneSetData(object):
    ...
    # ------------------------------------------------------------------
    # These helpers normalize and merge gene-level signal sources
    ...
    def read_Y(self, gwas_in=None, huge_statistics_in=None, huge_statistics_out=None, exomes_in=None, positive_controls_in=None, positive_controls_list=None, case_counts_in=None, ctrl_counts_in=None, gene_bfs_in=None, gene_loc_file=None, gene_covs_in=None, hold_out_chrom=None, **kwargs):
        ...
        if exomes_in is not None:
            ...
            if self.genes is None:
                ...
                #extra_genes_exomes and extra_Y_exomes have genes not yet in self.genes
        ...
        if case_counts_in is not None or ctrl_counts_in is not None:
            ...
            if self.genes is None:
                ...
                self._set_X(self.X_orig, extra_genes_case_counts, self.gene_sets, skip_N=True, skip_V=True)
                #set this temporarily for use in huge
                self.Y_case_counts = extra_Y_case_counts
                ...
                #exomes and positive controls are already aligned to self.genes: Y1_exomes/Y1_positive_controls match self.genes
                #extra_genes_all / extra_Y_exomes / extra_genes_positive_controls / extra_Y_positive_controls has anything not in it
                #we need to:
        ...
        #5. extra_Y: the total combined gwas + exome + positive_controls + case_counts values for all genes in either gwas or exomes or positive controls or case counts but not in self.genes
        #6. extra_Y_exomes: the total exome values for all genes in either gwas or exomes or positive controls or case counts but not in self.genes
        #7. extra_Y_positive_controls: the total positive control values for all genes in either gwas or exomes or positive controls or case counts but not in self.genes
        #8. extra_Y_case_counts: the total case counts values for all genes in either gwas or exomes or positive controls or case counts but not in self.genes
        ...
        if self.genes is None:
            ...
            (Y, Y_for_regression, Y_exomes, Y_positive_controls, Y_case_counts, extra_genes, extra_Y, extra_Y_for_regression, extra_Y_exomes, extra_Y_positive_controls, extra_Y_case_counts) = self._initialize_y_from_new_gene_universe(
            ...
            (Y, Y_for_regression, Y_exomes, Y_positive_controls, Y_case_counts, extra_genes, extra_Y, extra_Y_for_regression, extra_Y_exomes, extra_Y_positive_controls, extra_Y_case_counts) = self._merge_y_into_existing_gene_universe(
                Y1=Y1,
        ...
        self._set_Y(Y, Y_for_regression, Y_exomes, Y_positive_controls, Y_case_counts, skip_V=True, skip_scale_factors=True)
    ...
    def run_gibbs(self, max_num_iter=100, total_num_iter=None, max_num_restarts=3, num_chains=10, num_mad=3, r_threshold_burn_in=1.10, use_max_r_for_convergence=True, increase_hyper_if_betas_below=None, update_huge_scores=True, top_gene_prior=None, min_num_burn_in=10, max_num_burn_in=None, min_num_post_burn_in=None, max_num_post_burn_in=None, max_num_iter_betas=1100, min_num_iter_betas=10, num_chains_betas=4, r_threshold_burn_in_betas=1.01, use_max_r_for_convergence_betas=True, max_frac_sem_betas=0.01, use_mean_betas=True, warm_start=False, burn_in_rhat_quantile=0.95, burn_in_patience=2, burn_in_stall_window=10, burn_in_stall_delta=0.01, stop_mcse_quantile=0.95, stop_patience=2, stop_top_gene_k=200, stop_min_gene_d=None, max_abs_mcse_d=0.05, max_rel_mcse_beta=0.20, active_beta_top_k=200, active_beta_min_abs=0.01, beta_rel_mcse_denom_floor=0.10, stall_window=8, stall_min_burn_in=50, stall_min_post_burn_in=50, stall_delta_rhat=0.01, stall_delta_mcse=0.01, stall_recent_window=4, stall_recent_eps=0.0, stopping_preset_name="lenient", diag_every=5, sparse_frac_gibbs=0.01, sparse_max_gibbs=0.001, sparse_solution=False, sparse_frac_betas=None, pre_filter_batch_size=None, pre_filter_small_batch_size=500, max_allowed_batch_correlation=None, gauss_seidel_betas=False, gauss_seidel=False, num_batches_parallel=10, max_mb_X_h=200, initial_linear_filter=True, correct_betas_mean=True, correct_betas_var=True, adjust_priors=True, gene_set_stats_trace_out=None, gene_stats_trace_out=None, betas_trace_out=None, eps=0.01):
        ...
        while num_attempts < max_num_attempt_restarts and num_completed_epochs < target_num_epochs and remaining_total_iter > 0:
            ...
            sum_betas_m = epoch_state["sum_betas_m"]
            sum_betas2_m = epoch_state["sum_betas2_m"]
            sum_betas_uncorrected_m = epoch_state["sum_betas_uncorrected_m"]
            sum_betas_uncorrected2_m = epoch_state["sum_betas_uncorrected2_m"]
            sum_postp_m = epoch_state["sum_postp_m"]
            sum_beta_tildes_m = epoch_state["sum_beta_tildes_m"]
            sum_z_scores_m = epoch_state["sum_z_scores_m"]
            num_sum_beta_m = epoch_state["num_sum_beta_m"]
            sum_Ys_m = epoch_state["sum_Ys_m"]
            sum_Ys2_m = epoch_state["sum_Ys2_m"]
            sum_Y_raws_m = epoch_state["sum_Y_raws_m"]
            sum_log_pos_m = epoch_state["sum_log_pos_m"]
            sum_log_pos2_m = epoch_state["sum_log_pos2_m"]
            sum_log_po_raws_m = epoch_state["sum_log_po_raws_m"]
            sum_log_po_raws2_m = epoch_state["sum_log_po_raws2_m"]
            sum_priors_m = epoch_state["sum_priors_m"]
            sum_priors2_m = epoch_state["sum_priors2_m"]
            sum_Ds_m = epoch_state["sum_Ds_m"]
            sum_D_raws_m = epoch_state["sum_D_raws_m"]
            sum_bf_orig_m = epoch_state["sum_bf_orig_m"]
            sum_bf_uncorrected_m = epoch_state["sum_bf_uncorrected_m"]
            sum_bf_orig_raw_m = epoch_state["sum_bf_orig_raw_m"]
            sum_bf_orig_raw2_m = epoch_state["sum_bf_orig_raw2_m"]
            num_sum_Y_m = epoch_state["num_sum_Y_m"]
            ...
            in_burn_in = epoch_state["in_burn_in"]
            burn_in_pass_streak = epoch_state["burn_in_pass_streak"]
            burn_in_rhat_history = epoch_state["burn_in_rhat_history"]
            stop_pass_streak = epoch_state["stop_pass_streak"]
            R_beta_v = epoch_state["R_beta_v"]
            betas_sem2_v = epoch_state["betas_sem2_v"]
            sem2_v = epoch_state["sem2_v"]
            stop_due_to_stall = epoch_state["stop_due_to_stall"]
            stop_due_to_precision = epoch_state["stop_due_to_precision"]
            restart_due_to_stall = epoch_state["restart_due_to_stall"]
            burn_stall_best_beta_rhat_history = epoch_state["burn_stall_best_beta_rhat_history"]
            burn_stall_snapshots = epoch_state["burn_stall_snapshots"]
            burn_stall_beta_indices = epoch_state["burn_stall_beta_indices"]
            post_stall_best_beta_rhat_history = epoch_state["post_stall_best_beta_rhat_history"]
            post_stall_best_D_mcse_history = epoch_state["post_stall_best_D_mcse_history"]
            post_stall_snapshots = epoch_state["post_stall_snapshots"]
            post_stall_beta_indices = epoch_state["post_stall_beta_indices"]
            post_stall_gene_indices = epoch_state["post_stall_gene_indices"]
            ...
            sum_priors_missing_m = epoch_state["sum_priors_missing_m"]
            sum_Ds_missing_m = epoch_state["sum_Ds_missing_m"]
            priors_missing_sample_m = epoch_state["priors_missing_sample_m"]
            priors_missing_mean_m = epoch_state["priors_missing_mean_m"]
            num_sum_priors_missing_m = epoch_state["num_sum_priors_missing_m"]
            ...
            prev_Ys_m = epoch_state["prev_Ys_m"]
            ...
            for iteration_num in range(epoch_max_num_iter):
                ...
                iteration_progress_update = _advance_gibbs_iteration_progress(
                    ...
                    burn_stall_best_beta_rhat_history=burn_stall_best_beta_rhat_history,
                ...
                in_burn_in = iteration_progress_update["in_burn_in"]
                burn_in_pass_streak = iteration_progress_update["burn_in_pass_streak"]
                stop_pass_streak = iteration_progress_update["stop_pass_streak"]
                prev_Ys_m = iteration_progress_update["prev_Ys_m"]
                burn_stall_beta_indices = iteration_progress_update["burn_stall_beta_indices"]
                R_beta_v = iteration_progress_update["R_beta_v"]
                post_stall_beta_indices = iteration_progress_update["post_stall_beta_indices"]
                post_stall_gene_indices = iteration_progress_update["post_stall_gene_indices"]
                betas_sem2_v = iteration_progress_update["betas_sem2_v"]
                sem2_v = iteration_progress_update["sem2_v"]
                stop_due_to_precision = iteration_progress_update["stop_due_to_precision"]
                restart_due_to_stall = iteration_progress_update["restart_due_to_stall"]
                stop_due_to_stall = iteration_progress_update["stop_due_to_stall"]
        ...
        with open_gz(output_file, 'w') as output_fh:
            ...
            for i in ordered_i:
                ...
                if self.betas is not None:
                    ...
                    if self.betas_r_hat is not None:
                        line = "%s\t%.3g\t%.3g" % (line, self.betas_r_hat[i], self.betas_mcse[i])
                if self.betas_uncorrected is not None and not basic:
                    line = "%s\t%.3g" % (line, self.betas_uncorrected[i] / self.scale_factors[i])            
                    if self.betas_uncorrected_r_hat is not None:
                        line = "%s\t%.3g\t%.3g" % (line, self.betas_uncorrected_r_hat[i], self.betas_uncorrected_mcse[i])
                if not basic:
                    if self.non_inf_avg_cond_betas is not None:
            ...
            if self.gene_sets_missing is not None:
                ...
                if self.betas_missing is not None and self.scale_factors_missing is not None:
    ...
    def write_phewas_gene_set_statistics(self, output_file, max_no_write_gene_set_beta=None, max_no_write_gene_set_beta_uncorrected=None, basic=False):
        ...
        if self.p_values_phewas is None:
            log("No stats available; skipping", INFO)
            return
        with open_gz(output_file, 'w') as output_fh:
            if self.gene_sets is None:
                return
            ...
            if self.X_orig is not None:
                ...
                header = "%s\t%s" % (header, "N")
                header = "%s\t%s" % (header, "scale")
            if self.beta_tildes_phewas is not None:
                header = "%s\t%s\t%s\t%s\t%s\t%s" % (header, "beta_tilde", "beta_tilde_internal", "P", "Z", "SE")
            if self.betas_phewas is not None:
                header = "%s\t%s\t%s" % (header, "beta", "beta_internal")
            if self.betas_uncorrected_phewas is not None and not basic:
                header = "%s\t%s" % (header, "beta_uncorrected")            

            output_fh.write("%s\n" % header)

            for p in range(len(self.phenos)):

                ordered_i = range(len(self.gene_sets))
                if self.betas_uncorrected_phewas is not None:
                    ordered_i = sorted(ordered_i, key=lambda k: -self.betas_uncorrected_phewas[p,k] / self.scale_factors[k])
                elif self.p_values_phewas is not None:
                    ordered_i = sorted(ordered_i, key=lambda k: self.p_values_phewas[p,k])

                for i in ordered_i:

                    if max_no_write_gene_set_beta is not None and self.betas_phewas is not None and np.abs(self.betas_phewas[p,i] / self.scale_factors[i]) <= max_no_write_gene_set_beta:
                        continue

                    if max_no_write_gene_set_beta_uncorrected is not None and self.betas_uncorrected_phewas is not None and np.abs(self.betas_uncorrected_phewas[p,i] / self.scale_factors[i]) <= max_no_write_gene_set_beta_uncorrected:
                        continue
                    ...
                    if self.gene_set_labels is not None:
                        line = "%s\t%s" % (line, self.gene_set_labels[i])
                    if self.phenos is not None:
                        line = "%s\t%s" % (line, self.phenos[p])
                    if self.X_orig is not None:
                        line = "%s\t%d" % (line, col_sums[i])
                        line = "%s\t%.3g" % (line, self.scale_factors[i])

                    if self.beta_tildes_phewas is not None:
                        line = "%s\t%.3g\t%.3g\t%.3g\t%.3g\t%.3g" % (line, self.beta_tildes_phewas[p,i] / self.scale_factors[i], self.beta_tildes_phewas[p,i], self.p_values_phewas[p,i], self.z_scores_phewas[p,i], self.ses_phewas[p,i] / self.scale_factors[i])
                    if self.betas_phewas is not None:
                        line = "%s\t%.3g\t%.3g" % (line, self.betas_phewas[p,i] / self.scale_factors[i], self.betas_phewas[p,i])
                    if self.betas_uncorrected_phewas is not None and not basic:
    ...
    def write_gene_statistics(self, output_file):
        ...
        with open_gz(output_file, 'w') as output_fh:
            ...
            if self.Y is not None and self.Y_r_hat is not None and self.Y_mcse is not None and self.genes is not None:
                for i in range(len(self.genes)):
                    if np.isfinite(self.Y_mcse[i]) and np.isfinite(self.Y_r_hat[i]) and self.Y_mcse[i] > 0 and self.Y_r_hat[i] > 1:
            ...
            if self.gene_to_case_count_logbf is not None:
            ...
            for i in ordered_i:
                ...
                if self.priors is not None:
                    ...
                    if self.priors_r_hat is not None:
                        line = "%s\t%.3g\t%.3g" % (line, self.priors_r_hat[i], self.priors_mcse[i])
                if self.priors_adj is not None:
                    line = "%s\t%.3g" % (line, self.priors_adj[i])
                if self.combined_prior_Ys is not None:
                    line = "%s\t%.3g" % (line, self.combined_prior_Ys[i])
                    if self.combined_prior_Ys_r_hat is not None:
                        line = "%s\t%.3g\t%.3g" % (line, self.combined_prior_Ys_r_hat[i], self.combined_prior_Ys_mcse[i])
                if self.combined_prior_Ys_adj is not None:
                ...
                if self.gene_to_case_count_logbf is not None:
                    if gene in self.gene_to_case_count_logbf:
                        line = "%s\t%.3g" % (line, self.gene_to_case_count_logbf[gene])
                    else:
                        line = "%s\t%s" % (line, "NA")
                if self.Y is not None:
                    ...
                    if write_log_bf_diagnostics:
                        if np.isfinite(self.Y_mcse[i]) and np.isfinite(self.Y_r_hat[i]) and self.Y_mcse[i] > 0 and self.Y_r_hat[i] > 1:
                            line = "%s\t%.3g\t%.3g" % (line, self.Y_r_hat[i], self.Y_mcse[i])
                        else:
                            line = "%s\t%s\t%s" % (line, "NA", "NA")
                if write_regression:
...
def _write_huge_statistics_bundle(runtime_state, prefix, gene_bf, extra_genes, extra_gene_bf, gene_bf_for_regression, extra_gene_bf_for_regression):
    ...
    matrix_row_genes = _build_huge_statistics_matrix_row_genes(cache_genes, extra_genes, huge_signal_bfs.shape[0])
    (gene_to_score, gene_to_score_uncorrected, gene_to_score_for_regression) = _build_huge_statistics_score_maps(
        runtime_state,
        cache_genes,
        extra_genes,
        gene_bf,
    ...
    meta = _build_huge_statistics_meta(runtime_state, huge_signal_bfs, huge_signal_bfs_for_regression)

    with open_gz(paths["meta"], 'w') as out_fh:
        json.dump(meta, out_fh, sort_keys=True)
        out_fh.write("\n")

    _write_huge_statistics_text_tables(
        paths,
        runtime_state,
        cache_genes,
        extra_genes,
    ...
    _write_huge_statistics_runtime_vectors(paths, runtime_state)
    _write_huge_statistics_sparse_components(paths, huge_signal_bfs, huge_signal_bfs_for_regression)
...
def _read_huge_statistics_bundle(runtime_state, prefix):
    ...
    (
        ...
        gene_to_score_uncorrected,
        gene_to_score_for_regression,
    ) = _read_huge_statistics_text_tables(paths)

    (gene_bf, gene_bf_for_regression) = _resolve_huge_statistics_gene_vectors(
        runtime_state,
        cache_genes,
        extra_genes,
        matrix_row_genes,
    ...
    _load_huge_statistics_sparse_and_vectors(runtime_state, paths, meta)
    _apply_huge_statistics_meta_to_runtime(runtime_state, meta)
    _read_huge_statistics_covariates_if_present(runtime_state, paths)
    ...
    _combine_runtime_huge_scores(runtime_state)
    _validate_huge_statistics_loaded_shapes(runtime_state, matrix_row_genes)
...
def _run_main_core(state, options, mode_state, sigma2_cond, Y_not_loaded):
    ...
    _resolve_main_beta_tildes(state, options, mode_state)
