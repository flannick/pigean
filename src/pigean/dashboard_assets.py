...
function phenotypeRows(eaggl) { const rows = []; for (const factor of eaggl.factors || []) { for (const pheno of factor.phenotypes || []) rows.push({...pheno, factor_label: factor.label || factor.factor}); } rows.sort((a,b) => (Number(b.joint_ln_bf) || -1e300) - (Number(a.joint_ln_bf) || -1e300) || (Number(b.joint_capture_fraction) || 0) - (Number(a.joint_capture_fraction) || 0)); return rows; }
function renderPhenotypes(eaggl) { const rows = phenotypeRows(eaggl); const cols = [{key:"trait",label:"Trait"},{key:"factor",label:"Factor"},{key:"factor_label",label:"Factor label"},{key:"joint_posterior_lift",label:"Joint lift",numeric:true},{key:"joint_ln_bf",label:"Joint lnBF",numeric:true},{key:"joint_notable",label:"Joint notable"},{key:"marginal_posterior_lift",label:"Marginal lift",numeric:true},{key:"marginal_ln_bf",label:"Marginal lnBF",numeric:true},{key:"anchor_conditional_posterior_lift",label:"Anchor-cond lift",numeric:true},{key:"anchor_conditional_ln_bf",label:"Anchor-cond lnBF",numeric:true},{key:"trait_neff",label:"Trait neff",numeric:true},{key:"retained_fraction",label:"Retained frac",numeric:true},{key:"joint_capture_fraction",label:"Capture: joint frac",numeric:true},{key:"marginal_capture_fraction",label:"Capture: marginal frac",numeric:true},{key:"joint_capture_residual",label:"Capture: residual",numeric:true},{key:"trait_linkage_evidence_source",label:"Evidence"},{key:"is_anchor",label:"Anchor"}]; return `<h2>EAGGL phenotype loadings</h2><p class="note">Rows come from trait_factor_links.out.gz after the dashboard trait effective-size filter. Lift and lnBF columns are Bayesian effect/evidence scores; capture columns are the older allocation/projection diagnostics.</p>${tableHtml(`phenotypes:${eaggl.run_id}:${eaggl.mode_id}`, rows, cols, {pageSize: 20})}`; }
