...
function selectedGeneSource(eaggl) { const sources = geneLoadingSources(eaggl); const defaultId = DATA.default_gene_loading_source || "full_direct"; return sources.find(s => s.id === state.geneLoadingSource) || sources.find(s => s.id === defaultId) || sources.find(s => s.id === "full_direct") || sources.find(s => s.id === "full_via_gene_sets") || sources.find(s => s.id === "discovery") || sources[0] || null; }
