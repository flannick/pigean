...
.factor-graph-grid { display: grid; grid-template-columns: minmax(0, 1fr) 340px; gap: 12px; align-items: stretch; }
.dashboard-factor-graph { width: 100%; height: 720px; border: 1px solid var(--line); border-radius: 12px; background: #fff; }
.dashboard-factor-graph .graph-edge { stroke-opacity: 0.42; transition: stroke-opacity 0.12s, stroke-width 0.12s; }
.dashboard-factor-graph .graph-node { cursor: pointer; transition: opacity 0.12s; }
.dashboard-factor-graph .graph-node:hover circle, .dashboard-factor-graph .graph-node:hover rect, .dashboard-factor-graph .graph-node:hover polygon { stroke-width: 3; }
.graph-provenance-panel { border: 1px solid var(--line); border-radius: 12px; padding: 12px; background: #fbfdfc; overflow: auto; max-height: 720px; }
.graph-provenance-panel table th { position: static; }
.graph-selected { outline: 3px solid rgba(15,118,110,0.35); }
.graph-controls { display: grid; grid-template-columns: minmax(180px,1fr) minmax(180px,1fr) auto auto auto; gap: 10px; align-items: end; margin: 10px 0 12px; }
.graph-controls label { margin: 0; }
.graph-controls input[type="checkbox"] { width: auto; margin-right: 6px; }
.graph-status { color: var(--muted); font-size: 12px; grid-column: 1 / -1; }
...
function isSelectedPhiRun(r) { const value = String((r.selected_phi_metrics || {}).selected ?? "").toLowerCase(); return value === "1" || value === "1.0" || value === "true" || value === "yes"; }
function selectedEaggl(runId) { const runs = eagglForRun(runId); const explicit = runs.find(r => r.mode_id === state.modeId); if (explicit) return explicit; const selected = runs.find(isSelectedPhiRun); return selected || runs[0] || null; }
...
function factorDetailRows(eaggl, f, table) { if (table === "genes") return {title:"Top gene loadings", rows:factorGenesFromSource(eaggl, f), cols:[{key:"gene",label:"Gene"},{key:"loading",label:"Loading",numeric:true},{key:"relative_loading",label:"Relative",numeric:true},{key:"combined_loading",label:"Combined",numeric:true},{key:"combined",label:"Gene combined",numeric:true}]}; return {title:"Top gene set loadings", rows:f.gene_sets || [], cols:[{key:"gene_set",label:"Gene set"},{key:"label",label:"Label"},{key:"loading",label:"Loading",numeric:true},{key:"relative_loading",label:"Relative",numeric:true},{key:"combined_loading",label:"Combined",numeric:true},{key:"beta",label:"Beta",numeric:true}]}; }
...
function groupMetricHeatmap(runId, activeModeId) {
  ...
  const body = rows.map(r => {
    ...
    const selected = String(r.metrics.selected ?? r.selected_phi_metrics?.selected ?? "").toLowerCase();
    const isSelected = selected === "1" || selected === "true" || selected === "yes";
    const cells = available.map(m => {
      ...
      const text = value === null ? "NA" : `${m.composite && isSelected ? "★ " : ""}${fmt(value)}`;
      return `<td class="phi-metric-cell ${best ? "best" : ""} ${m.composite ? "metric-composite" : ""}">${text}</td>`;
    }).join("");
    return `<tr class="${active ? "phi-metric-row-active" : ""}"><th>${isSelected ? "★ " : ""}${esc(r.title || r.mode_id)}</th>${cells}</tr>`;
  }).join("");
...
function selectedFactorGraph(eaggl) { const source = selectedGeneSource(eaggl); if (source?.factor_graph_available) return {data: source.factor_graph_data || null, html: source.factor_graph_html || "", path: source.factor_graph_json_path || source.factor_graph_html_path, label: source.label}; return {data: eaggl?.factor_graph_data || null, html: eaggl?.factor_graph_html || "", path: eaggl?.paths?.factor_graph_json || eaggl?.paths?.factor_graph_html || "", label: source?.label || "default"}; }
function graphHasData(graph) { return graph?.data?.nodes && graph.data.nodes.length; }
function renderFactorGraphSlot(eaggl) { const graph = selectedFactorGraph(eaggl); if (!graphHasData(graph) && !graph.html) return `<details class="factor-graph-accordion"><summary>Factor graph</summary><div class="factor-graph-body empty">No factor graph was generated for this EAGGL run and gene-loading source.</div></details>`; return `<div id="factor-graph-slot" data-graph-key="${esc(factorGraphKey(eaggl))}"></div>`; }
function graphNodeRadius(node) { return Math.max(6, Number(node.radius) || ((Number(node.size) || 20) / 5)); }
function graphNodeShape(node, x, y, radius) { const fill = esc(node.color || "#9ca3af"), stroke = esc(node.border_color || node.color || "#4b5563"); if (node.shape === "square") { const s = radius * 2; return `<rect data-node-shape="1" x="${(x - radius).toFixed(2)}" y="${(y - radius).toFixed(2)}" width="${s.toFixed(2)}" height="${s.toFixed(2)}" rx="4" fill="${fill}" stroke="${stroke}" stroke-width="2.5"></rect>`; } if (node.shape === "diamond") { const s = radius; return `<polygon data-node-shape="1" points="${x.toFixed(2)},${(y-s).toFixed(2)} ${(x+s).toFixed(2)},${y.toFixed(2)} ${x.toFixed(2)},${(y+s).toFixed(2)} ${(x-s).toFixed(2)},${y.toFixed(2)}" fill="${fill}" stroke="${stroke}" stroke-width="2.5"></polygon>`; } return `<circle data-node-shape="1" cx="${x.toFixed(2)}" cy="${y.toFixed(2)}" r="${radius.toFixed(2)}" fill="${fill}" stroke="${stroke}" stroke-width="2.5"></circle>`; }
function graphScaledPositions(graph, width=1200, height=760) { const nodes = graph.nodes || []; const xs = nodes.map(n => Number(n.x)).filter(Number.isFinite), ys = nodes.map(n => Number(n.y)).filter(Number.isFinite); const minX = xs.length ? Math.min(...xs) : 0, maxX = xs.length ? Math.max(...xs) : 1, minY = ys.length ? Math.min(...ys) : 0, maxY = ys.length ? Math.max(...ys) : 1; const pad = 80, dx = Math.max(1e-9, maxX - minX), dy = Math.max(1e-9, maxY - minY); const pos = {}; for (const n of nodes) pos[n.id] = {x: pad + (Number(n.x) - minX) / dx * (width - 2 * pad), y: height - (pad + (Number(n.y) - minY) / dy * (height - 2 * pad))}; return {pos, width, height}; }
function prepareGraphRuntime(graph) { const scaled = graphScaledPositions(graph); graph.viewport = {width: scaled.width, height: scaled.height}; for (const node of graph.nodes || []) { const p = scaled.pos[node.id] || {x: scaled.width / 2, y: scaled.height / 2}; node._x = Number.isFinite(node._x) ? node._x : p.x; node._y = Number.isFinite(node._y) ? node._y : p.y; node._fixedX = p.x; node._fixedY = p.y; node._radius = graphNodeRadius(node); node._vx = node._vx || 0; node._vy = node._vy || 0; } return scaled; }
function renderGraphDataSvg(graph) { const {width, height} = prepareGraphRuntime(graph); const edges = (graph.edges || []).map((edge, ix) => { const title = `${edge.from} → ${edge.to} | weight ${fmtText(edge.weight)} | field ${edge.provenance?.weight_field || "loading"}`; return `<line class="graph-edge" data-graph-edge-index="${ix}" data-source="${esc(edge.from)}" data-target="${esc(edge.to)}" stroke="${esc(edge.color || "#94a3b8")}" stroke-width="${Math.max(0.4, Number(edge.width) || 1).toFixed(2)}"><title>${esc(title)}</title></line>`; }).join(""); const nodes = (graph.nodes || []).map(node => { const label = node.display_label || node.label || node.id; const title = `${node.kind}: ${node.label || node.id}`; return `<g class="graph-node" data-graph-node-id="${esc(node.id)}" opacity="0.96"><title>${esc(title)}</title>${graphNodeShape(node, node._x, node._y, node._radius)}<text data-node-label="1" x="${node._x.toFixed(2)}" y="${(node._y + node._radius + 13).toFixed(2)}" text-anchor="middle" font-size="11" fill="#1f2933">${esc(label)}</text></g>`; }).join(""); return `<svg class="dashboard-factor-graph" viewBox="0 0 ${width} ${height}" role="img" aria-label="EAGGL factor graph"><g id="dashboard-graph-edges">${edges}</g><g id="dashboard-graph-nodes">${nodes}</g></svg>`; }
function graphEntityLookup(eaggl, id) { for (const factor of eaggl.factors || []) { if (factor.factor === id) return {kind:"factor", row:factor}; for (const gene of factorGenesFromSource(eaggl, factor)) if (gene.gene === id) return {kind:"gene", row:gene, factor}; for (const pheno of factor.phenotypes || []) if ((pheno.trait || pheno.id) === id) return {kind:"trait", row:pheno, factor}; } return {kind:"node", row:{id}}; }
function graphProvenanceHtml(eaggl, nodeId) { const found = graphEntityLookup(eaggl, nodeId); const row = found.row || {}; if (found.kind === "factor") { return `<h3>${esc(row.label || row.factor)}</h3><p><strong>Factor:</strong> ${esc(row.factor)}</p><p><strong>Lambda:</strong> ${fmt(row.lambda)} · <strong>Mass:</strong> ${fmt(row.combined_mass_fraction)}</p><h3>Top genes</h3>${smallTable(factorGenesFromSource(eaggl, row).slice(0,5), [{key:"gene",label:"Gene"},{key:"loading",label:"Loading",numeric:true},{key:"combined",label:"Combined",numeric:true}], `graph:${eaggl.mode_id}:${nodeId}:genes`)}<h3>Top gene sets</h3>${smallTable((row.gene_sets || []).slice(0,5), [{key:"gene_set",label:"Gene set"},{key:"loading",label:"Loading",numeric:true},{key:"beta",label:"Beta",numeric:true}], `graph:${eaggl.mode_id}:${nodeId}:sets`)}`; } if (found.kind === "gene") { return `<h3>${esc(row.gene || nodeId)}</h3><p><strong>Loading:</strong> ${fmt(row.loading)} · <strong>Factor:</strong> ${esc(found.factor?.label || found.factor?.factor || "")}</p><p><strong>Combined:</strong> ${fmt(row.combined)} · <strong>Direct:</strong> ${fmt(row.log_bf)} · <strong>Prior:</strong> ${fmt(row.prior)}</p>`; } if (found.kind === "trait") { return `<h3>${esc(row.trait || nodeId)}</h3><p><strong>Factor:</strong> ${esc(found.factor?.label || found.factor?.factor || "")}</p><p><strong>Joint fraction:</strong> ${fmt(row.joint_fraction)} · <strong>Trait Neff:</strong> ${fmt(row.trait_neff)}</p>`; } return `<h3>${esc(nodeId)}</h3><p class="note">No dashboard provenance row found for this node.</p>`; }
function createFactorGraphNode(eaggl) { const graph = selectedFactorGraph(eaggl); const wrapper = document.createElement("div"); wrapper.id = "factor-graph-mounted"; wrapper.dataset.graphKey = factorGraphKey(eaggl); if (graphHasData(graph)) { wrapper.innerHTML = `<details class="factor-graph-accordion" open><summary>Factor graph</summary><div class="factor-graph-body"><p class="note">Graph rendered from shared dashboard graph data for ${esc(graph.label)}. Node provenance is resolved from dashboard tables.</p><div class="graph-controls"><label>Filter nodes<input id="graphFilterInput" type="text" placeholder="e.g. PPARG or beta"></label><label>Add gene node<input id="graphAddNodeInput" list="graphAddNodeOptions" type="text" placeholder="search embedded genes"><datalist id="graphAddNodeOptions"></datalist></label><button id="graphAddNodeButton" type="button">Add</button><label><input id="graphHideUnmatched" type="checkbox">Hide unmatched</label><label><input id="graphPhysicsToggle" type="checkbox" checked>Physics</label><button id="graphResetButton" type="button">Reset</button><div id="graphStatus" class="graph-status"></div></div><div class="factor-graph-grid">${renderGraphDataSvg(graph.data)}<aside id="graph-provenance-panel" class="graph-provenance-panel"><h3>Graph provenance</h3><p class="note">Click a node to inspect the corresponding dashboard record. Drag nodes to adjust layout. Hover an edge to see weight and source field.</p></aside></div><p class="path">${esc(graph.path || "")}</p></div></details>`; return wrapper; } const graphFrame = graph.html ? `<iframe class="factor-graph-frame" srcdoc="${esc(graph.html)}"></iframe>` : ""; wrapper.innerHTML = graphFrame ? `<details class="factor-graph-accordion" open><summary>Factor graph</summary><div class="factor-graph-body"><p class="note">Interactive graph generated from ${esc(graph.label)} gene loadings.</p>${graphFrame}<p class="path">${esc(graph.path || "")}</p></div></details>` : `<details class="factor-graph-accordion"><summary>Factor graph</summary><div class="factor-graph-body empty">No factor graph was generated for this EAGGL run and gene-loading source.</div></details>`; return wrapper; }
function graphPointer(svg, event) { const pt = svg.createSVGPoint(); pt.x = event.clientX; pt.y = event.clientY; return pt.matrixTransform(svg.getScreenCTM().inverse()); }
function updateGraphDom(graph) { const nodeById = new Map((graph.nodes || []).map(n => [n.id, n])); document.querySelectorAll(".graph-edge").forEach(el => { const edge = (graph.edges || [])[Number(el.dataset.graphEdgeIndex)]; const a = edge ? nodeById.get(edge.from) : null, b = edge ? nodeById.get(edge.to) : null; if (!a || !b) { el.style.display = "none"; return; } el.setAttribute("x1", a._x); el.setAttribute("y1", a._y); el.setAttribute("x2", b._x); el.setAttribute("y2", b._y); el.style.display = (a._visible === false || b._visible === false) ? "none" : ""; el.style.opacity = (a._dimmed || b._dimmed) ? "0.14" : "1"; }); document.querySelectorAll(".graph-node").forEach(g => { const node = nodeById.get(g.dataset.graphNodeId); if (!node) return; const shape = g.querySelector("[data-node-shape]"); const text = g.querySelector("[data-node-label]"); if (shape?.tagName === "circle") { shape.setAttribute("cx", node._x); shape.setAttribute("cy", node._y); shape.setAttribute("r", node._radius); } else if (shape?.tagName === "rect") { shape.setAttribute("x", node._x - node._radius); shape.setAttribute("y", node._y - node._radius); shape.setAttribute("width", node._radius * 2); shape.setAttribute("height", node._radius * 2); } else if (shape?.tagName === "polygon") { const r=node._radius, x=node._x, y=node._y; shape.setAttribute("points", `${x},${y-r} ${x+r},${y} ${x},${y+r} ${x-r},${y}`); } if (text) { text.setAttribute("x", node._x); text.setAttribute("y", node._y + node._radius + 13); } g.style.display = node._visible === false ? "none" : ""; g.style.opacity = node._dimmed ? "0.18" : "0.96"; }); }
function dashboardGraphCandidateRows(eaggl, graph) { const shown = new Set((graph.nodes || []).map(n => n.id)); const rows = loadingMatrixRows(eaggl, "genes").filter(r => !shown.has(r.gene)); rows.sort((a,b) => (Number(b.max_loading)||0) - (Number(a.max_loading)||0)); return rows; }
function addDashboardGeneNode(eaggl, graph, query) { const q = String(query || "").trim().toLowerCase(); const rows = dashboardGraphCandidateRows(eaggl, graph); const row = rows.find(r => String(r.gene).toLowerCase() === q) || rows.find(r => `${r.gene} ${r.label || ""}`.toLowerCase().includes(q)); const status = byId("graphStatus"); if (!row) { if (status) status.textContent = "No matching embedded gene loading row."; return; } const factorById = new Map((eaggl.factors || []).map(f => [f.factor, f])); const factorNodeById = new Map((graph.nodes || []).filter(n => n.kind === "factor").map(n => [n.id, n])); const entries = Object.entries(row.values || {}).map(([factor, vals]) => [factor, Number(vals.loading) || 0]).filter(([,v]) => v > 0); if (!entries.length) return; const total = entries.reduce((s, [,v]) => s + v, 0); let x = 0, y = 0; for (const [factor, v] of entries) { const fn = factorNodeById.get(factor); if (fn) { x += fn._x * v / total; y += fn._y * v / total; } } const top = entries.reduce((a,b) => b[1] > a[1] ? b : a, entries[0]); const topNode = factorNodeById.get(top[0]); const node = {id: row.gene, label: row.label || row.gene, display_label: row.gene, kind: "gene", shape: "circle", color: topNode?.color || "#78909c", border_color: topNode?.border_color || topNode?.color || "#546e7a", _x: x || 600, _y: y || 380, _fixedX: x || 600, _fixedY: y || 380, _radius: 10 + 18 * Math.min(1, Number(row.max_loading) || 0)}; graph.nodes.push(node); for (const [factor, loading] of entries.filter(([,v]) => v >= 0.01)) graph.edges.push({from: factor, to: row.gene, kind:"factor_gene", weight: loading, width: Math.max(0.5, 5 * loading), color: factorNodeById.get(factor)?.color || "#94a3b8", provenance:{source_table:"dashboard_gene_loading", weight:loading, weight_field:factor}}); if (status) status.textContent = `Added ${row.gene}`; const mounted = byId("factor-graph-mounted"); if (mounted) mounted.replaceWith(createFactorGraphNode(eaggl)); mountFactorGraph(eaggl, null); }
function applyDashboardGraphFilter(graph) { const q = String(byId("graphFilterInput")?.value || "").toLowerCase(); const hide = Boolean(byId("graphHideUnmatched")?.checked); let visible = 0; for (const node of graph.nodes || []) { const match = !q || `${node.id} ${node.label || ""} ${node.kind}`.toLowerCase().includes(q); node._visible = match || !hide; node._dimmed = !match && !!q; if (node._visible) visible += 1; } const status = byId("graphStatus"); if (status) status.textContent = `${visible} / ${(graph.nodes || []).length} nodes visible`; updateGraphDom(graph); }
function bindFactorGraphEvents(eaggl) { const graph = selectedFactorGraph(eaggl).data; const panel = byId("graph-provenance-panel"); const svg = document.querySelector(".dashboard-factor-graph"); if (!panel || !svg || !graph) return; let dragging = null, panning = false, last = null, physicsTimer = null; let viewBox = {x: 0, y: 0, w: Number(graph.viewport?.width) || 1200, h: Number(graph.viewport?.height) || 760}; const setViewBox = () => svg.setAttribute("viewBox", `${viewBox.x} ${viewBox.y} ${viewBox.w} ${viewBox.h}`); setViewBox(); const refreshOptions = () => { const dl = byId("graphAddNodeOptions"); if (!dl) return; dl.innerHTML = dashboardGraphCandidateRows(eaggl, graph).slice(0,1000).map(r => `<option value="${esc(r.gene)}" label="${esc(r.label || r.gene)}"></option>`).join(""); }; const tick = () => { if (!byId("graphPhysicsToggle")?.checked) return; const nodeById = new Map((graph.nodes || []).map(n => [n.id,n])); for (const edge of graph.edges || []) { const a=nodeById.get(edge.from), b=nodeById.get(edge.to); if (!a || !b) continue; const dx=b._x-a._x, dy=b._y-a._y, dist=Math.max(1, Math.hypot(dx,dy)); const desired=85 + 20 / Math.max(0.05, Number(edge.weight)||0.05); const force=0.003*(dist-desired); const fx=force*dx/dist, fy=force*dy/dist; if (!a._pinned) { a._vx=(a._vx||0)+fx; a._vy=(a._vy||0)+fy; } if (!b._pinned) { b._vx=(b._vx||0)-fx; b._vy=(b._vy||0)-fy; } } for (let i=0;i<(graph.nodes||[]).length;i++) for (let j=i+1;j<graph.nodes.length;j++) { const a=graph.nodes[i], b=graph.nodes[j]; const dx=b._x-a._x, dy=b._y-a._y, d2=Math.max(100, dx*dx+dy*dy), d=Math.sqrt(d2), f=140/d2; const fx=f*dx/d, fy=f*dy/d; if (!a._pinned) { a._vx=(a._vx||0)-fx; a._vy=(a._vy||0)-fy; } if (!b._pinned) { b._vx=(b._vx||0)+fx; b._vy=(b._vy||0)+fy; } } for (const n of graph.nodes || []) { if (n._pinned) continue; n._vx=((n._vx||0)+(n._fixedX-n._x)*0.002)*0.84; n._vy=((n._vy||0)+(n._fixedY-n._y)*0.002)*0.84; n._x += n._vx; n._y += n._vy; } updateGraphDom(graph); physicsTimer = requestAnimationFrame(tick); }; document.querySelectorAll(".graph-node[data-graph-node-id]").forEach(g => { g.onpointerdown = event => { dragging = (graph.nodes || []).find(n => n.id === g.dataset.graphNodeId); if (!dragging) return; dragging._pinned = true; last = graphPointer(svg, event); g.setPointerCapture?.(event.pointerId); event.preventDefault(); }; g.onclick = event => { event.preventDefault(); document.querySelectorAll(".graph-node").forEach(n => n.classList.remove("graph-selected")); g.classList.add("graph-selected"); panel.innerHTML = graphProvenanceHtml(eaggl, g.dataset.graphNodeId); bind(); }; }); svg.onpointerdown = event => { if (event.target.closest && event.target.closest(".graph-node")) return; panning = true; last = graphPointer(svg, event); svg.setPointerCapture?.(event.pointerId); event.preventDefault(); }; svg.onpointermove = event => { if (!last) return; const pt = graphPointer(svg, event); if (dragging) { dragging._x += pt.x - last.x; dragging._y += pt.y - last.y; dragging._fixedX = dragging._x; dragging._fixedY = dragging._y; last = pt; updateGraphDom(graph); return; } if (panning) { viewBox.x -= pt.x - last.x; viewBox.y -= pt.y - last.y; setViewBox(); last = graphPointer(svg, event); } }; svg.onpointerup = () => { dragging = null; panning = false; last = null; }; svg.onpointerleave = () => { dragging = null; panning = false; last = null; }; svg.onwheel = event => { event.preventDefault(); const pt = graphPointer(svg, event); const scale = event.deltaY < 0 ? 0.9 : 1.1; viewBox.x = pt.x - (pt.x - viewBox.x) * scale; viewBox.y = pt.y - (pt.y - viewBox.y) * scale; viewBox.w *= scale; viewBox.h *= scale; setViewBox(); }; const filter = byId("graphFilterInput"); if (filter) filter.oninput = () => applyDashboardGraphFilter(graph); const hide = byId("graphHideUnmatched"); if (hide) hide.onchange = () => applyDashboardGraphFilter(graph); const add = byId("graphAddNodeButton"); if (add) add.onclick = () => addDashboardGeneNode(eaggl, graph, byId("graphAddNodeInput")?.value); const addInput = byId("graphAddNodeInput"); if (addInput) addInput.onkeydown = e => { if (e.key === "Enter") addDashboardGeneNode(eaggl, graph, addInput.value); }; const reset = byId("graphResetButton"); if (reset) reset.onclick = () => { for (const n of graph.nodes || []) { n._x=n._fixedX; n._y=n._fixedY; n._vx=0; n._vy=0; n._pinned=false; } updateGraphDom(graph); }; refreshOptions(); applyDashboardGraphFilter(graph); if (byId("graphPhysicsToggle")?.checked) tick(); }
function mountFactorGraph(eaggl, existingGraph) { const slot = document.getElementById("factor-graph-slot"); if (!slot) return; slot.replaceWith(existingGraph || createFactorGraphNode(eaggl)); bindFactorGraphEvents(eaggl); }
...
function renderEagglPanel(runId) {
  ...
  const graphAvailable = graphHasData(graphInfo) || Boolean(graphInfo.html);
  ...
  const groupedControls = meaningfulGroups
    ? `<div class="controls">${groupControl}</div>${groupMetricHeatmap(runId, eaggl.mode_id)}<div class="controls">${modeControl}${sourceControl}<div><span class="pill">${esc(graphAvailable ? "Factor graph available" : "No factor graph")}</span></div></div>`
    : `<div class="controls">${modeControl}${sourceControl}<div><span class="pill">${esc(graphAvailable ? "Factor graph available" : "No factor graph")}</span></div></div>`;
...
function render() { const run = selectedRun(); if (!run) { byId("app").innerHTML = `<section class="panel"><div class="empty">No PIGEAN or EAGGL runs were supplied.</div></section>`; return; } const eaggl = selectedEaggl(run.run_id); const existingGraph = detachMountedFactorGraph(eaggl); byId("app").innerHTML = renderControls(run, eaggl) + renderPigeanTables(run) + renderEagglPanel(run.run_id); bind(); bindHeatmapTooltips(); mountFactorGraph(eaggl, existingGraph); restoreFocus(); }
...
function refreshEagglTable() { const region = byId("eaggl-table-region"); const run = selectedRun(); const eaggl = run ? selectedEaggl(run.run_id) : null; if (!region || !eaggl) return false; region.innerHTML = renderEagglTableRegion(eaggl); bind(); bindHeatmapTooltips(); restoreFocus(); return true; }
