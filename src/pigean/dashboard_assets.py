...
function renderEagglTableRegion(eaggl) { const anchorCols = (eaggl.anchor_traits || []).map(anchor => ({key:anchor.column,label:anchor.label || anchor.trait,numeric:true,render:r=>fmt(r[anchor.column])})); const rows = (eaggl.factors || []).map(f => ({...f, mass:f.combined_mass_fraction, n_genes:factorGenesFromSource(eaggl, f).length, n_gene_sets:(f.gene_sets || []).length, n_traits:(f.phenotypes || []).length})); const cols = [{key:"label",label:"Mechanism",render:r=>`${esc(r.label || r.factor)} <span class="badge">${esc(r.factor)}</span>`},...anchorCols,{key:"lambda",label:"Lambda",numeric:true},{key:"mass",label:"Mass",numeric:true},{key:"top_genes",label:"Top gene loadings",filterValue:r=>factorGenesFromSource(eaggl, r).map(g=>`${g.gene} ${g.label || ""}`).join(" "),render:(r,k)=>factorInlineDetails(eaggl,r,"genes",k)},{key:"top_gene_sets",label:"Top gene set loadings",filterValue:r=>(r.gene_sets || []).map(gs=>`${gs.gene_set} ${gs.label || ""}`).join(" "),render:(r,k)=>factorInlineDetails(eaggl,r,"gene_sets",k)}]; const factorTableId = `factors:${eaggl.run_id}:${eaggl.mode_id}`; if (state.eagglSection === "genes") return loadingHeatmap(eaggl, "genes"); if (state.eagglSection === "gene_sets") return loadingHeatmap(eaggl, "gene_sets"); if (state.eagglSection === "phenotypes") return renderPhenotypes(eaggl); return `<h2>EAGGL factors</h2><p class="note">Factor detail subtables inherit parent filters. Phenotype projections are available in the dedicated Phenotypes tab.</p>${tableHtml(factorTableId, rows, cols, {hasDetail: factorHasDetail, detailRenderer:(row,key)=>factorRowDetails(eaggl,row,key)})}`; }
