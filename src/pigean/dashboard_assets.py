...
function factorDetailRows(eaggl, f, table) { if (table === "genes") return {title:"Top gene loadings", rows:factorGenesFromSource(eaggl, f), cols:[{key:"gene",label:"Gene"},{key:"loading",label:"Loading",numeric:true},{key:"cosine_loading",label:"Cosine",numeric:true},{key:"combined",label:"Gene combined",numeric:true}]}; return {title:"Top gene set loadings", rows:f.gene_sets || [], cols:[{key:"gene_set",label:"Gene set"},{key:"label",label:"Label"},{key:"loading",label:"Loading",numeric:true},{key:"cosine_loading",label:"Cosine",numeric:true},{key:"beta",label:"Beta",numeric:true}]}; }
...
function renderPhenotypes(eaggl) { const rows = phenotypeRows(eaggl); const has = key => rows.some(r => r[key] !== null && r[key] !== undefined && r[key] !== ""); const cols = [{key:"trait",label:"Trait"},{key:"factor",label:"Factor"},{key:"factor_label",label:"Factor label"},{key:"nnls_loading",label:"NNLS",numeric:true}]; if (has("beta")) cols.push({key:"beta",label:"Beta",numeric:true}); if (has("beta_uncorrected")) cols.push({key:"beta_uncorrected",label:"Beta uncorr",numeric:true}); if (has("beta_tilde")) cols.push({key:"beta_tilde",label:"Beta tilde",numeric:true}); if (has("se")) cols.push({key:"se",label:"SE",numeric:true}); if (has("z")) cols.push({key:"z",label:"Z",numeric:true}); if (has("p_value")) cols.push({key:"p_value",label:"P",numeric:true}); cols.push({key:"trait_neff",label:"Trait neff",numeric:true},{key:"trait_response_source",label:"Response"},{key:"is_anchor",label:"Anchor"}); return `<h2>EAGGL phenotype loadings</h2><p class="note">Rows come from trait_factor_links.out.gz. NNLS is the native EAGGL projection of the trait support vector onto factor gene loadings. PIGEAN beta statistics should be generated separately by exporting factors with --factor-gmt-out and running the PIGEAN multi-Y beta workflow.</p>${tableHtml(`phenotypes:${eaggl.run_id}:${eaggl.mode_id}`, rows, cols, {pageSize: 20})}`; }
