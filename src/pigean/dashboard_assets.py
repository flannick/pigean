...
function factorRowDetails(eaggl, f, rowKey) { const tab = state.factorDetailTabs[rowKey] || "genes"; const detail = factorDetailRows(eaggl, f, tab); const parentTableId = rowKey.split(":").slice(0, -1).join(":"); const labels = {genes:"Genes", gene_sets:"Gene sets", phenotypes:"Phenotypes"}; const tabs = ["genes", "gene_sets", "phenotypes"].map(name => `<button data-factor-detail-tab="${esc(rowKey)}" data-tab="${esc(name)}" class="${tab === name ? "active" : ""}">${labels[name]}</button>`).join(""); return `<div class="details-box factor-details"><div class="factor-tabs">${tabs}</div>${smallTable(detail.rows, detail.cols, `${rowKey}:${tab}:table`, nestedFilterOptions(parentTableId, tab))}</div>`; }
