...
function renderPhenotypes(eaggl) { const rows = phenotypeRows(eaggl); const cols = [{key:"trait",label:"Trait"},{key:"factor",label:"Factor"},{key:"factor_label",label:"Factor label"},{key:"nnls_loading",label:"NNLS",numeric:true},{key:"beta",label:"Beta",numeric:true},{key:"beta_uncorrected",label:"Beta uncorr",numeric:true},{key:"beta_tilde",label:"Beta tilde",numeric:true},{key:"se",label:"SE",numeric:true},{key:"z",label:"Z",numeric:true},{key:"p_value",label:"P",numeric:true},{key:"trait_neff",label:"Trait neff",numeric:true},{key:"trait_response_source",label:"Response"},{key:"is_anchor",label:"Anchor"}]; return `<h2>EAGGL phenotype loadings</h2><p class="note">Rows come from trait_factor_links.out.gz. NNLS is the projection of the trait support vector onto factor gene loadings; beta columns treat EAGGL factors as weighted gene sets.</p>${tableHtml(`phenotypes:${eaggl.run_id}:${eaggl.mode_id}`, rows, cols, {pageSize: 20})}`; }
