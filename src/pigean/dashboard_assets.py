...
.loading-heatmap thead th { height: 30px; position: sticky; top: 0; z-index: 3; }
...
.loading-heatmap thead .heat-row-label { z-index: 4; text-transform: uppercase; letter-spacing: 0.06em; font-size: 11px; }
.loading-heatmap .heat-factor { width: 18px; min-width: 18px; cursor: pointer; }
...
function selectedGeneSource(eaggl) { const sources = geneLoadingSources(eaggl); return sources.find(s => s.id === state.geneLoadingSource) || sources.find(s => s.id === "discovery") || sources[0] || null; }
...
function factorDetailRows(eaggl, f, table) { if (table === "genes") return {title:"Top gene loadings", rows:factorGenesFromSource(eaggl, f), cols:[{key:"gene",label:"Gene"},{key:"loading",label:"Loading",numeric:true},{key:"relative_loading",label:"Relative",numeric:true},{key:"combined_loading",label:"Combined",numeric:true},{key:"combined",label:"Gene combined",numeric:true}]}; if (table === "gene_sets") return {title:"Top gene set loadings", rows:f.gene_sets || [], cols:[{key:"gene_set",label:"Gene set"},{key:"label",label:"Label"},{key:"loading",label:"Loading",numeric:true},{key:"relative_loading",label:"Relative",numeric:true},{key:"combined_loading",label:"Combined",numeric:true},{key:"beta",label:"Beta",numeric:true}]}; return {title:"PheWAS", rows:f.phenotypes || [], cols:[{key:"trait",label:"Trait"},{key:"joint_fraction",label:"Joint frac",numeric:true},{key:"marginal_fraction",label:"Marginal",numeric:true},{key:"trait_neff",label:"Trait neff",numeric:true},{key:"joint_support_mass",label:"Joint support",numeric:true},{key:"joint_residual",label:"Residual",numeric:true}]}; }
function factorInlineDetails(eaggl, f, table, rowKey) { const detail = factorDetailRows(eaggl, f, table); return `<button data-open-row-tab="${esc(rowKey)}" data-tab="${esc(table)}">${state.openRows[rowKey] && state.factorDetailTabs[rowKey] === table ? "Hide" : "Show"} ${esc(detail.title.toLowerCase())} (${detail.rows.length})</button>`; }
function factorHasDetail(f, rowKey) { return Boolean(state.openRows[rowKey]); }
function nestedFilterOptions(parentTableId, nestedTable) { const parentKey = nestedTable === "genes" ? "top_genes" : nestedTable === "gene_sets" ? "top_gene_sets" : "phewas"; const value = (state.columnFilters[parentTableId] || {})[parentKey] || ""; if (!value) return {}; const childKey = nestedTable === "genes" ? "gene" : nestedTable === "gene_sets" ? "gene_set" : "trait"; return {presetFilters: {[childKey]: value}}; }
function factorRowDetails(eaggl, f, rowKey) { const tab = state.factorDetailTabs[rowKey] || "genes"; const detail = factorDetailRows(eaggl, f, tab); const parentTableId = rowKey.split(":").slice(0, -1).join(":"); const tabs = ["genes", "gene_sets", "phenotypes"].map(name => { const d = factorDetailRows(eaggl, f, name); return `<button data-factor-detail-tab="${esc(rowKey)}" data-tab="${esc(name)}" class="${tab === name ? "active" : ""}">${esc(d.title)} (${d.rows.length})</button>`; }).join(""); return `<div class="details-box factor-details"><div class="tabs compact-tabs">${tabs}</div><div class="subpanel full-width-subpanel"><h3>${esc(detail.title)}</h3>${smallTable(detail.rows, detail.cols, `${rowKey}:${tab}:table`, nestedFilterOptions(parentTableId, tab))}</div></div>`; }
...
function loadingHeatmap(eaggl, kind) { const rows = loadingMatrixRows(eaggl, kind); const idKey = kind === "genes" ? "gene" : "gene_set"; const factors = eaggl.factors || []; const tableId = `heatmap:${kind}:${eaggl.run_id}:${eaggl.mode_id}:${selectedGeneSource(eaggl)?.id || "default"}`; const metric = state.heatmapMetric || "loading"; const sortFactor = state.heatmapSorts[tableId] || ""; const maxValue = Math.max(0, ...rows.flatMap(row => Object.values(row.values || {}).map(v => Number(v[metric]) || 0))); const sorted = [...rows].sort((a,b) => sortFactor ? (metricValue({[metric]:(b.values[sortFactor] || {})[metric]}, metric) - metricValue({[metric]:(a.values[sortFactor] || {})[metric]}, metric)) : ((b.max_loading || 0) - (a.max_loading || 0) || String(a[idKey]).localeCompare(String(b[idKey])))); const title = kind === "genes" ? "EAGGL gene loading heatmap" : "EAGGL gene-set loading heatmap"; const metricLabel = metric === "relative_loading" ? "relative" : metric === "combined_loading" ? "combined" : "raw"; const controls = `<div class="heatmap-controls"><label>Loading metric <select id="heatmapMetricSelect"><option value="loading" ${metric === "loading" ? "selected" : ""}>Raw</option><option value="relative_loading" ${metric === "relative_loading" ? "selected" : ""}>Relative</option><option value="combined_loading" ${metric === "combined_loading" ? "selected" : ""}>Combined</option></select></label><span class="note">${rows.length} embedded rows · click a column to sort</span></div>`; const head = factors.map(f => `<th class="heat-factor" data-heatmap-table="${esc(tableId)}" data-factor="${esc(f.factor)}" title="${esc(f.label || f.factor)}"><span></span></th>`).join(""); const body = sorted.map(row => `<tr><th class="heat-row-label" title="${esc(row.label || row[idKey])}">${esc(row[idKey])}</th>${factors.map(f => { const vals = row.values[f.factor] || {}; const value = Number(vals[metric]) || 0; const tip = `${row[idKey]} · ${f.label || f.factor} · ${metricLabel}: ${fmtText(value)}`; return `<td class="heat-cell" title="${esc(tip)}" style="background:${heatColor(value, maxValue)}"><span>${value > 0 ? fmt(value) : ""}</span></td>`; }).join("")}</tr>`).join("") || `<tr><td class="empty">No rows available.</td></tr>`; return `<h2>${title}</h2><p class="note">Rows are dashboard-embedded ${kind === "genes" ? "genes" : "gene sets"}; columns are factors and show labels on hover.</p>${controls}<div class="heatmap-wrap"><table class="loading-heatmap"><thead><tr><th class="heat-row-label">${kind === "genes" ? "Gene" : "Gene set"}</th>${head}</tr></thead><tbody>${body}</tbody></table></div>`; }
...
function renderEagglTableRegion(eaggl) { const anchorCols = (eaggl.anchor_traits || []).map(anchor => ({key:anchor.column,label:anchor.label || anchor.trait,numeric:true,render:r=>fmt(r[anchor.column])})); const rows = (eaggl.factors || []).map(f => ({...f, mass:f.combined_mass_fraction, n_genes:factorGenesFromSource(eaggl, f).length, n_gene_sets:(f.gene_sets || []).length, n_traits:(f.phenotypes || []).length})); const cols = [{key:"label",label:"Mechanism",render:r=>`${esc(r.label || r.factor)} <span class="badge">${esc(r.factor)}</span>`},{key:"factor_tier",label:"Tier"},...anchorCols,{key:"lambda",label:"Lambda",numeric:true},{key:"mass",label:"Mass",numeric:true},{key:"top_genes",label:"Top gene loadings",filterValue:r=>factorGenesFromSource(eaggl, r).map(g=>`${g.gene} ${g.label || ""}`).join(" "),render:(r,k)=>factorInlineDetails(eaggl,r,"genes",k)},{key:"top_gene_sets",label:"Top gene set loadings",filterValue:r=>(r.gene_sets || []).map(gs=>`${gs.gene_set} ${gs.label || ""}`).join(" "),render:(r,k)=>factorInlineDetails(eaggl,r,"gene_sets",k)},{key:"phewas",label:"PheWAS",filterValue:r=>(r.phenotypes || []).map(p=>p.trait).join(" "),render:(r,k)=>factorInlineDetails(eaggl,r,"phenotypes",k)}]; const factorTableId = `factors:${eaggl.run_id}:${eaggl.mode_id}`; if (state.eagglSection === "genes") return loadingHeatmap(eaggl, "genes"); if (state.eagglSection === "gene_sets") return loadingHeatmap(eaggl, "gene_sets"); return `<h2>EAGGL factors</h2><p class="note">Anchor-trait columns show joint fraction when multi-trait links are available. Factor detail subtables inherit parent filters.</p>${tableHtml(factorTableId, rows, cols, {hasDetail: factorHasDetail, detailRenderer:(row,key)=>factorRowDetails(eaggl,row,key)})}`; }
function renderEagglPanel(runId) { const eaggl = selectedEaggl(runId); const modes = eagglForRun(runId); if (!eaggl) return `<section class="panel"><h2>EAGGL factors</h2><div class="empty">No EAGGL run was supplied for this PIGEAN run.</div></section>`; const sources = geneLoadingSources(eaggl); const currentSource = selectedGeneSource(eaggl); if (currentSource && state.geneLoadingSource !== currentSource.id) state.geneLoadingSource = currentSource.id; const modeControl = modes.length > 1 ? `<select id="modeSelect">${modes.map(m => `<option value="${esc(m.mode_id)}" ${m.mode_id === eaggl.mode_id ? "selected" : ""}>${esc(m.title || m.mode_id)}</option>`).join("")}</select>` : `<div class="pill">${esc(eaggl.title || eaggl.mode_id)}</div>`; const graphInfo = selectedFactorGraph(eaggl); const sourceControl = sources.length > 1 ? `<div><label>Gene loading source</label><select id="geneLoadingSourceSelect">${sources.map(s => `<option value="${esc(s.id)}" ${s.id === state.geneLoadingSource ? "selected" : ""}>${esc(s.label)}</option>`).join("")}</select></div>` : ""; return `<section class="panel" id="eaggl-panel"><div class="controls"><div><label>EAGGL run</label>${modeControl}</div>${sourceControl}<div><span class="pill">${esc(graphInfo.html ? "Factor graph available" : "No factor graph")}</span></div></div><p class="lede">${esc(eaggl.summary || "EAGGL run supplied on the dashboard command line.")}</p><p class="path">${esc(eaggl.paths?.factors || "missing factors")}<br>${esc(currentSource?.path || "")}</p>${warningsHtml(eaggl.warnings)}${renderFactorGraphSlot(eaggl)}<div class="tabs"><button data-eaggl-section="factors" class="${state.eagglSection === "factors" ? "active" : ""}">Factors</button><button data-eaggl-section="genes" class="${state.eagglSection === "genes" ? "active" : ""}">Genes</button><button data-eaggl-section="gene_sets" class="${state.eagglSection === "gene_sets" ? "active" : ""}">Gene sets</button></div><div id="eaggl-table-region">${renderEagglTableRegion(eaggl)}</div></section>`; }
...
function refreshEagglTable() { const region = byId("eaggl-table-region"); const run = selectedRun(); const eaggl = run ? selectedEaggl(run.run_id) : null; if (!region || !eaggl) return false; region.innerHTML = renderEagglTableRegion(eaggl); bind(); restoreFocus(); return true; }
function refreshTableForKey(key) { if (String(key).startsWith("factors:") || String(key).startsWith("heatmap:") || String(key).includes(":gene_by_")) return refreshEagglTable(); if (String(key).startsWith("genes:") || String(key).startsWith("gene_sets:")) return refreshPigeanTable(); return refreshPigeanTable() || refreshEagglTable(); }
