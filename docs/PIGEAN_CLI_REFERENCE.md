...
The gene-level PheWAS output stage (`--run-phewas`) is deprecated expert compatibility functionality. Prefer native `--multi-y-in` for trait-wise PIGEAN beta workflows and EAGGL trait-linkage outputs for factor-to-trait summaries. If this legacy stage is used, the default is the matched comparison set; use `--phewas-comparison-set diagnostic` only when you explicitly want the cross-family diagnostics.
