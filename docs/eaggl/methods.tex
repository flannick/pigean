...
When anchor phenotypes are present and canonical trait linkage is also computed, EAGGL reports `anchor_any_joint` and `anchor_any_marginal` summaries in `factors.out(.gz)` derived from the canonical linkage calculations in Section~\ref{sec:postfactor_revised}. The per-anchor `factors_anchor.out(.gz)` table reports the corresponding `joint` and `marginal` values for each anchor separately. During the transition period, `any_relevance` remains as a compatibility field and coincides with the anchor-any joint projection coefficient summary in those phenotype-anchored linkage runs.
...
\item `joint_coefficient` = $j_{tk}^{(b)}$
\item `marginal_coefficient` = $m_{tk}^{(b)}$
\item `marginal_overlap` = $o_{tk}^{(b)}$
\item `joint_coefficient_support_mass` = `trait_total_support` $\times$ `joint_coefficient`, a coefficient-scaled support total rather than an exact captured-support mass
\item `marginal_coefficient_support_mass` = `trait_total_support` $\times$ `marginal_coefficient`, a coefficient-scaled support total rather than an exact captured-support mass
\item `marginal_overlap_support_mass` = `trait_total_support` $\times$ `marginal_overlap`, an overlap-scaled support total
\item `factor_total_mass` = $S_k^{(b)}$ in the factor summary output
\item `factor_n_eff` = $\left(\sum_f W_{fk}^{(b)}\right)^2 \big/ \sum_f \left(W_{fk}^{(b)}\right)^2$
\item `factor_top_share` = $\max_f W_{fk}^{(b)} / S_k^{(b)}$
\item `factor_top10_share` = top-10 loading mass divided by $S_k^{(b)}$
\item `broad_factor_flag` = 1 when `factor_n_eff` $\ge 500$ and `factor_top_share` $\le 0.01$
\end{itemize}
...
\begin{equation}
\frac{E_{j-1} - E_j}{C_j - C_{j-1}} \ge \eta,
\label{eq:eaggl_marginal_gain}
\end{equation}
where $\eta > 0$ is a fixed marginal-gain threshold. The selected value of $\phi$ is the largest-complexity frontier candidate reached before this marginal gain drops below $\eta$. If no uncapped candidate is acceptable, the same frontier rule is applied to the capped candidates. A separate fit-loss tolerance is retained only for fallback ranking when no candidate satisfies the primary redundancy and restart criteria.
