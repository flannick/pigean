...
After fitting, EAGGL reports factor-level relevance scores that summarize how strongly each factor is supported by the selected trait or anchor set. These are downstream summaries derived from the fitted factor basis and are not themselves part of the factorization objective. In non-phenotype-anchored workflows, this implicit factor relevance remains the legacy bounded nonnegative regression summary used for the `any_relevance` column in `factors.out(.gz)`.
...
When anchor phenotypes are present and canonical trait linkage is also computed, EAGGL additionally reports `anchor_joint`, `anchor_marginal`, `anchor_any_joint`, and `anchor_any_marginal` summaries derived from the canonical linkage calculations in Section~\ref{sec:postfactor_revised}. During the transition period, `any_relevance` remains as a compatibility field and coincides with the anchor-any joint summary in those phenotype-anchored linkage runs.
...
\[
p_t^{(b)} \in \mathbb{R}_+^{F_b}
\]
denote the thresholded trait-support profile for phenotype $t$ on that same basis.
...
For linkage, EAGGL first computes the total thresholded trait strength on the full thresholded support surface, before applying the factor mask. Define the total thresholded trait strength
\begin{equation}
a_t^{(b)} = \sum_{f=1}^{F_b^{\mathrm{full}}} p_{tf}^{(b,\mathrm{full})},
\label{eq:eaggl_pheno_strength}
\end{equation}
where $p_t^{(b,\mathrm{full})}$ denotes the full thresholded support surface on basis $b$ before masking. Let $\mathcal{M}_b$ denote the retained factor mask on that basis, and let
\[
p_{t,\mathcal{M}}^{(b)}
\]
denote the masked thresholded support profile restricted to the retained factor basis rows. The NNLS solve is performed on this masked profile, but the normalization denominator remains the full thresholded trait strength. Thus the normalized masked trait profile is
\begin{equation}
\tilde{p}_{t,\mathcal{M}}^{(b)} = \frac{p_{t,\mathcal{M}}^{(b)}}{a_t^{(b)} + \epsilon},
...
EAGGL then reports two linkage summaries from the same normalized inputs. The \emph{marginal} linkage of factor $k$ to trait $t$ is the one-factor bounded projection
\begin{equation}
m_{tk}^{(b)}
=
\arg\min_{0 \le c \le 1}
\left\|
\tilde{p}_{t,\mathcal{M}}^{(b)} - \tilde{B}^{(b)}_{\bullet k} c
\right\|_2^2,
\label{eq:eaggl_trait_linkage_marginal}
\end{equation}
and the \emph{joint} linkage is the all-factor competitive projection
\begin{equation}
j_t^{(b)}
=
\arg\min_{c \ge 0,\ \mathbf{1}^\top c \le 1}
\left\|
\tilde{p}_{t,\mathcal{M}}^{(b)} - \tilde{B}^{(b)} c
\right\|_2^2.
...
The coefficient $m_{tk}^{(b)}$ is interpreted as the standalone relevance of factor $k$ to phenotype $t$ when that factor is viewed alone. The coefficient $j_{tk}^{(b)}$ is interpreted as the unique explanatory relevance of factor $k$ to phenotype $t$ after the other factors compete. In contrast to a projection normalized only on the retained masked slice, Eqs.~\eqref{eq:eaggl_trait_linkage_marginal} and \eqref{eq:eaggl_trait_linkage_joint} preserve the scale of the full thresholded trait while still solving on the retained factor basis. Broad phenotypes can therefore have large total thresholded strength $a_t^{(b)}$ without automatically receiving inflated linkage weights when only a small masked subset of their support survives into the factor basis.

\paragraph{Interpretation.}
Canonical trait linkage is a descriptive quantity. The marginal weights answer:
\begin{quote}
``How much does factor $k$ link to phenotype $t$ when factor $k$ is viewed alone?''
\end{quote}
The joint weights answer:
\begin{quote}
...
EAGGL also reports retained-basis diagnostics alongside the linkage scores: total thresholded trait strength, retained masked trait strength, retained fraction, total thresholded feature count, retained masked feature count, and a low-retention flag for traits with very small retained support.

\subsection{Choice of projection basis across workflow families}

For the default gene $\times$ gene-set workflow family (F1--F5), the preferred basis for canonical trait linkage is the gene basis. In that case, $B^{(b)}$ is the gene-by-factor loading matrix and $p_t^{(b)}$ is the thresholded phenotype-specific gene-support vector. If gene-level support is unavailable or gene-level factor loadings are absent, EAGGL falls back to the gene-set basis, in which $B^{(b)}$ is the gene-set-by-factor loading matrix and $p_t^{(b)}$ is the thresholded phenotype-specific gene-set support vector.

For the gene-set $\times$ phenotype workflow family (F6--F9), public trait linkage still uses the same projection definition whenever an external trait profile is provided, typically on the gene-set basis. Thus, the canonical linkage definition is common across workflows: EAGGL always projects one thresholded support profile onto a factor basis defined on one shared feature space, then reports both marginal and joint weights from those same normalized inputs.
...
\begin{itemize}
\item \textbf{Canonical trait linkage} gives bounded marginal and joint summaries of how well the retained high-confidence phenotype profile is represented by each factor. This is the primary annotation layer for interpreting factors with external phenotypes.
