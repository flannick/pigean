...
\subsection{Bayesian trait-factor effect and evidence scores}

The projection coefficients above are capture metrics: the joint coefficients are constrained to sum to at most one across factors, and the marginal coefficients are normalized-overlap scores. EAGGL therefore also reports Bayesian effect/evidence scores in the original selected trait-support units. These scores use a separate evidence surface $Y$ chosen by \texttt{--trait-factor-linkage-evidence-source}; in \texttt{auto} mode EAGGL prefers \texttt{log\_bf}, then \texttt{combined}, then \texttt{prior}. The selected surface can be used raw, thresholded, or converted to excess-over-threshold support.

For each factor $k$, EAGGL constructs a soft membership vector $M_k$ from the nonnegative factor loadings, either by dividing by the factor maximum or by using capped raw loadings. The marginal lift for trait $t$ is
\[
\Delta^{\mathrm{marginal}}_{tk}
=
\frac{\sum_f M_{fk}Y_{ft}}{\sum_f M_{fk}+\epsilon}
-
\frac{\sum_f (1-M_{fk})Y_{ft}}{\sum_f (1-M_{fk})+\epsilon}.
\]
It then fits the one-covariate model
\[
Y_t = \alpha + M_k\beta_{tk} + \varepsilon,
\qquad
\beta_{tk} \sim N(0,\sigma_\beta^2),
\]
and reports an approximate log Bayes factor, posterior lift, posterior standard deviation, and posterior probability that the lift is positive.

The joint effect score uses the same Bayesian one-covariate calculation after residualizing both $Y_t$ and $M_k$ against the intercept and all other factor memberships. This estimates the effect of factor $k$ conditional on the rest of the factor basis and is not constrained to sum to one. The anchor-conditional effect score similarly residualizes against available same-feature anchor support before testing $M_k$; if no matching anchor support is available, those columns are reported as unavailable rather than imputed.

The \texttt{*_notable} flags are conservative summaries requiring positive posterior lift, sufficient log Bayes factor, adequate trait effective size, adequate retained fraction, and no low-retention flag. They are Bayesian/effect/QC filters, not frequentist p-values.
