...
EAGGL then reports a single native phenotype-linkage summary: an NNLS projection of each selected trait support vector onto the factor basis,
\begin{equation}
\hat c_t^{(b)}
=
\arg\min_{c \ge 0}
...
\end{equation}
The resulting coefficient \texttt{nnls\_loading} is a descriptive projection loading. It answers how much of the thresholded trait-support profile is represented by each fitted factor on the shared feature basis. It is not a Bayes factor, p-value, posterior probability, or factor-as-gene-set regression coefficient.

For reporting, EAGGL writes:
\begin{itemize}
\item \texttt{nnls\_loading} = $\hat c_{tk}^{(b)}$
\item \texttt{trait\_neff} = $\left(\sum_f s_t^{(b)}\right)^2 \big/ \sum_f \left(s_t^{(b)}\right)^2$
\item \texttt{factor\_num\_genes}, the number of genes retained in the thresholded factor gene set
\item \texttt{factor\_weight\_sum}, the total retained factor gene weight
