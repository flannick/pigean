...
Suggested annotation exclusions are source-adaptive. EAGGL first identifies positive,
active annotations that are broad bridges across low-overlap factors, then compares
their separated bridge mass against both all annotations and annotations from the same
input source. Sources whose annotations empirically concentrate on one factor require
more extreme global bridge percentiles for automatic exclusion, whereas broad sources
use a less stringent global percentile. Small sources require an extreme global
percentile before automatic exclusion. This policy avoids hardcoded library names while
retaining a conservative review flag for source-specific outliers.
