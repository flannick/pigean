...
For large annotation collections, the implementation supports three budgeted approximation modes when the retained gene-set panel exceeds a user-specified budget. The legacy \emph{pruned} mode reduces the retained panel to at most the requested budget using the existing pruning routine, evaluates candidate $\phi$ values on that reduced panel, and reruns the final reported factorization on the reduced panel. This mode remains available as a fast baseline.

The \emph{online\_shared\_basis} mode preserves the full retained gene-set collection during scalable search and final factorization. In the standard transposed EAGGL factor matrix, retained gene sets are partitioned into blocks whose size is controlled by the same budget surface. One shared global gene-factor matrix and one shared ARD state are maintained across all blocks. For each block $B$, the implementation materializes the local submatrix $X_B$, solves the block-local gene-set-factor loadings with the shared global gene-factor basis held fixed, accumulates sufficient statistics from that block, and then performs a global update of the shared gene-factor matrix and the shared ARD weights after a full pass over all blocks. This block pass / global update cycle is repeated for a configurable number of epochs. After the final pass, the implementation reprojects all retained gene sets from scratch onto the final shared basis before writing the standard factor and cluster outputs. The fitted factor loadings are stored on their original model scale; no normalization is applied to the stored model.

The \emph{structure\_preserving\_sketch} mode instead chooses a sketch of at most the requested budget actual gene sets, factors that sketch with the exact EAGGL solver, and then projects all retained gene sets onto the learned basis. The sketch is therefore a subset of real columns rather than a collection of synthetic centroids. Any normalization used internally for sketch selection is computed only on transient copies and does not modify the stored fitted loadings. Optional short post-projection passes may be used with the basis fixed, but no separate factor-family merge or compressed-rank selection stage is introduced.

Candidate values are explored locally around the initial $\phi$ using multiplicative steps of two under a bounded total evaluation budget. The implementation can warm-start neighboring online candidates from the closest previously fitted $\phi$ on the log scale. These scalable approximations change only the cost of the search or fit procedure; they do not introduce an independent post hoc factor stitching model once $\phi$ has been chosen.
