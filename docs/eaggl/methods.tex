...
\usepackage{xr}
\usepackage{amsmath}
\IfFileExists{paper.aux}{\externaldocument{paper}}{}
...
names. Thus \texttt{flag\_review} is diagnostic, while \texttt{flag\_suggest\_exclude} is the single
...
After fitting the main factorization, EAGGL can relate latent factors to additional human phenotypes in two conceptually distinct ways. The primary public annotation layer is a descriptive \emph{trait linkage} step, which estimates how strongly each phenotype's probability-transformed support profile is represented by each factor using the same fixed-basis projection operator used for post-factorization gene and annotation projection. The secondary expert-only layer is an inferential \emph{enrichment} step, which asks whether a phenotype is more concentrated in one factor than expected after accounting for the anchor trait and optional covariates. These two quantities answer different scientific questions and are therefore not expected to coincide exactly.

\subsection{Canonical trait linkage by fixed-basis probability projection}

The external gene-by-phenotype support table can be very large. EAGGL therefore preserves sparse inputs by default. Publicly, EAGGL defaults to \texttt{--trait-linkage-source combined}; expert overrides can choose \texttt{log\_bf}, \texttt{prior}, or \texttt{auto} fallback resolution. Combined and log-BF values are treated as log Bayes factors and converted to probabilities with the run background prior,
\[
p_{tf}^{(b)} = \sigma\left(s_{tf}^{(b)} + \operatorname{logit}(\pi_0)\right),
\]
where $\pi_0$ is \texttt{background\_prior}. If the input phenotype matrix is sparse, only explicit entries are transformed and implicit zeros remain zero probability. If dense computation is requested, all entries are transformed, so a zero log Bayes factor maps to $\pi_0$. Prior-valued inputs are treated directly as probabilities after clipping to $[0,1]$.
...
\]
denote the selected trait-support profile before probability conversion. Let $p_t^{(b)}$ denote the probability-transformed trait profile on the same basis. The raw factor loadings $W^{(b)}$ are not normalized before projection.

For linkage diagnostics, EAGGL computes the total trait support on the full probability surface, before applying the factor mask. Define the total trait support
\begin{equation}
A_t^{(b)} = \sum_{f=1}^{F_b^{\mathrm{full}}} p_{tf}^{(b,\mathrm{full})},
\label{eq:eaggl_pheno_strength}
\end{equation}
where $p_t^{(b,\mathrm{full})}$ denotes the full probability surface on basis $b$ before masking. Let $\mathcal{M}_b$ denote the retained factor mask on that basis, and let
\[
p_{t,\mathcal{M}}^{(b)}
\]
denote the masked probability profile restricted to the retained factor basis rows.

EAGGL then reports a single native phenotype-linkage summary: a fixed-basis nonnegative projection of each selected trait probability vector onto the factor basis,
\begin{equation}
\hat c_t^{(b)}
=
\arg\min_{c \ge 0}
\left\|
p_{t,\mathcal{M}}^{(b)} - W_{\mathcal{M}}^{(b)} c
\right\|_2^2.
\label{eq:eaggl_trait_linkage_nnls}
\end{equation}
The resulting coefficient \texttt{nnls\_loading} is a descriptive projection loading. It answers how much of the probability-transformed trait-support profile is represented by each fitted factor on the shared feature basis. It is not a Bayes factor, p-value, posterior probability, or factor-as-gene-set regression coefficient.

For reporting, EAGGL writes:
\begin{itemize}
\item \texttt{nnls\_loading} = $\hat c_{tk}^{(b)}$
\item \texttt{trait\_neff} = $\left(\sum_f p_t^{(b)}\right)^2 \big/ \sum_f \left(p_t^{(b)}\right)^2$
\item \texttt{factor\_num\_genes}, the number of positive feature loadings in the factor basis
\item \texttt{factor\_weight\_sum}, the total factor feature weight
\item \texttt{trait\_response\_source}, the trait support surface used for projection
\item \texttt{factor\_gene\_basis}, the feature basis used for projection
\item \texttt{factor\_loading\_threshold}, the threshold used when exporting factor gene sets for PIGEAN enrichment
\end{itemize}

Regression-style factor-to-trait association is deliberately not estimated inside this linkage writer. When beta statistics are needed, EAGGL exports the thresholded factors with \texttt{--factor-gmt-out}; those factor gene sets should then be tested with PIGEAN \texttt{multi-y}, which writes \texttt{beta}, \texttt{beta\_uncorrected}, \texttt{beta\_tilde}, standard errors, z-scores, and p-values using the same beta-stage machinery as ordinary gene sets.

\paragraph{Interpretation.}
Canonical trait linkage is descriptive. The NNLS loading answers:
\begin{quote}
``How strongly is phenotype $t$ represented by factor $k$ when the phenotype support vector is projected onto the learned factor basis?''
\end{quote}
These loadings are useful for dashboard annotation and factor browsing. They should not be interpreted as calibrated evidence that phenotype $t$ is formally associated with factor $k$.

\subsection{Choice of projection basis across workflow families}

For the public gene $\times$ gene-set workflow family, the preferred basis for canonical trait linkage is the gene basis. In that case, $W^{(b)}$ is the gene-by-factor loading matrix and $p_t^{(b)}$ is the probability-transformed phenotype-specific gene-support vector. If gene-level support is unavailable or gene-level factor loadings are absent, EAGGL falls back to the gene-set basis, in which $W^{(b)}$ is the gene-set-by-factor loading matrix and $p_t^{(b)}$ is the probability-transformed phenotype-specific gene-set support vector. Thus, the canonical linkage definition is common across workflows: EAGGL always projects one probability support profile onto a factor basis defined on one shared feature space.
...
\paragraph{Interpretation.}
Unlike the canonical NNLS linkage coefficients in Eq.~\eqref{eq:eaggl_trait_linkage_nnls}, the coefficients in Eq.~\eqref{eq:eaggl_factor_pheno_regression} are incremental effects. They answer the question:
\begin{quote}
...
The binary thresholded model is the default because it is defensible under sparse thresholded phenotype input and easier to interpret than a joint all-factor fit. EAGGL reports independent-observation HC3 standard errors by default. The native binary fitter does not currently implement chromosome-clustered uncertainty; HC3 does not correct covariance between genes. See Section~\ref{sec:gene_dependence_robustness} for the distinction from exported factor-GMT inference and dependence sensitivity checks. The resulting p-values are still approximate, because the thresholded phenotype-hit process is derived from a combined-support quantity that already reflects gene-set-mediated indirect support.
...
\begin{itemize}
\item \textbf{Canonical trait linkage} gives nonnegative fixed-basis NNLS projection coefficients describing how strongly the retained high-confidence phenotype profile links to each factor. This is the primary annotation layer for interpreting factors with external phenotypes.
...
\input{gene_dependence_robustness.tex}
...
Although the model is defined in terms of weighted nonnegative factorization, a direct implementation that explicitly constructs separate dense matrices for every anchor user would be inefficient. EAGGL therefore uses the weighted objective of Eq.~\eqref{eq:eaggl_weighted_likelihood} directly, rather than materializing large user-specific tensors. This preserves the model while substantially reducing memory use. When a full-gene cluster table is requested, the default output projection routes genes through the fitted gene-set loadings; the direct gene-gene projection remains available as an explicitly selected diagnostic.
...
\paragraph{Geometric cluster summaries.}
Cluster output tables contain the raw factor loading for each row, a cosine column for each factor, and a Euclidean-distance column for each factor. For row loading vector $v_i$, the reported \texttt{Cosine\_Factor}$k$ value is $v_{ik}/\|v_i\|_2$, i.e. the cosine similarity between the row's full factor-loading vector and the one-hot indicator for factor $k$. The reported \texttt{Euclidean\_Factor}$k$ value is $\|v_i-e_k\|_2$, where $e_k$ is the one-hot indicator for factor $k$.
