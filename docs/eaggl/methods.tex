...
We decompose each thresholded profile into an overall retained strength and a compositional shape. Define the retained trait strength
\begin{equation}
a_t^{(b)} = \sum_{f=1}^{F_b} p_{tf}^{(b)},
\label{eq:eaggl_pheno_strength}
\end{equation}
and the corresponding normalized trait profile
\begin{equation}
\tilde{p}_t^{(b)} = \frac{p_t^{(b)}}{a_t^{(b)} + \epsilon},
...
\left\|
\tilde{p}_t^{(b)} - \tilde{B}^{(b)}_{\bullet k} c
\right\|_2^2,
...
The coefficient $m_{tk}^{(b)}$ is interpreted as the standalone relevance of factor $k$ to phenotype $t$ when that factor is viewed alone. The coefficient $j_{tk}^{(b)}$ is interpreted as the unique explanatory relevance of factor $k$ to phenotype $t$ after the other factors compete. In contrast to a projection on raw support mass, Eqs.~\eqref{eq:eaggl_trait_linkage_marginal} and \eqref{eq:eaggl_trait_linkage_joint} separate retained trait breadth from trait-specific shape. Broad phenotypes can therefore have large retained strength $a_t^{(b)}$ without automatically receiving high linkage weights for every factor.
