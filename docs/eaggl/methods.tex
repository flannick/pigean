...
In the implementation, the selected $\phi$, the candidate-level search diagnostics, and the thresholds used for the search are written to the run log and parameter output so that the final reported factorization is auditable after the fact. For very large annotation collections, the implementation may optionally evaluate candidate $\phi$ values on a sentinel subset of relatively uncorrelated gene sets and under a smaller candidate iteration budget, then rerun the final reported factorization on the full retained matrix with the selected $\phi$. This shortcut changes only the cost of the search procedure; it does not change the final fitting target once $\phi$ has been chosen.
