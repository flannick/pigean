...
% Add this section to docs/eaggl/methods.tex after the existing single-trait
% gene-by-annotation factorization / redundancy-balanced discovery material and
% before phenotype-anchored extensions or downstream trait-linkage sections.
...
\end{equation}
denote the anchor-specific shared annotation log-evidence. The default multi-anchor
gene-by-gene model first converts each anchor to a calibrated target matrix
$M_{ij}^{(t)}$ using the same pairwise probability transform described below. It then
estimates one shared loading matrix $W$ with a weighted multi-view objective,
\begin{equation}
\frac{1}{2}
\sum_t \sum_{i<j} C_{ij}^{(t)}
\left(
M_{ij}^{(t)} - \sum_k W_{ik}W_{jk}
\right)^2
+ \operatorname{ARD}(W),
\label{eq:eaggl_gene_gene_multi_view}
\end{equation}
where $C_{ij}^{(t)}$ is an anchor-specific confidence weight. By default, this
confidence is proportional to the anchor-specific target itself, so an anchor with no
evidence for an edge has zero or near-zero fitting weight for that edge rather than
acting as negative evidence against another anchor. If anchor-specific gene relevance
probabilities are available, EAGGL treats the pairwise direct-support gate
\begin{equation}
G_{ij}^{(t)} = p_i^{(t)}p_j^{(t)}
\label{eq:eaggl_gene_gene_multi_anchor_gate}
\end{equation}
as additional observation confidence for that anchor. Operationally, EAGGL forms the
sufficient statistics
\begin{equation}
C_{ij}^{\mathrm{sum}} = \sum_t C_{ij}^{(t)}, \qquad
M_{ij}^{\mathrm{eff}} =
\frac{\sum_t C_{ij}^{(t)} M_{ij}^{(t)}}{C_{ij}^{\mathrm{sum}}}
\quad\text{when } C_{ij}^{\mathrm{sum}}>0,
\label{eq:eaggl_gene_gene_multi_view_sufficient_stats}
\end{equation}
sets $M_{ij}^{\mathrm{eff}}=0$ when $C_{ij}^{\mathrm{sum}}=0$, and fits the weighted
symmetric objective with entry-wise weights $C_{ij}^{\mathrm{sum}}$.

An expert `any' mode instead combines anchor-specific target matrices as a noisy-OR
union,
\begin{equation}
M_{ij}^{\mathrm{any}} = 1 - \prod_t \left(1 - M_{ij}^{(t)}\right).
\label{eq:eaggl_gene_gene_any_anchor}
\end{equation}
This is a union target rather than a multi-view objective. Gene-by-annotation
multi-anchor EAGGL does not require a corresponding aggregation mode because its
rectangular model already pools anchor-derived weights into a single weighted
likelihood for one shared factor basis.
...
\end{equation}
Candidates must be non-collapsed, have enough primary factors, satisfy restart and redundancy constraints, and avoid severe underfit. The primary-factor definition is controlled by the configured mass floor fraction and is used consistently for primary-factor counts, primary-scoped redundancy and stability calculations, and target-size summaries. Restart stability is also matched over the same metric factor scope, which is primary by default and independent of the output-printing scope. Ordinary fit loss is treated as a warning-level diagnostic in target-size mode because in-sample reconstruction often favors over-fragmented lower-$\phi$ solutions; however, a separate severe-underfit threshold still blocks pathological high-$\phi$ candidates whose reconstruction error is far worse than the best candidate in the search, and candidates lacking a finite fit measurement are rejected while that severe guard is active. Among acceptable uncapped candidates with defined $S_{\phi}$, EAGGL first restricts to candidates within the configured size tolerance when any exist, then chooses the largest $\phi$ in that tolerance set. If none are in tolerance, it chooses the candidate with smallest $E_{\phi}$, using larger $\phi$, lower tail fraction, lower filtered fraction, lower redundancy, better fit, and lower primary-factor spike diagnostics as late tie-breakers. Candidate values are explored around the initial $\phi$ by target-aware bracketing: if fitted factors are too small, larger $\phi$ values are proposed; if fitted factors are too large, smaller $\phi$ values are proposed; once the target is bracketed, geometric midpoints refine the search.
