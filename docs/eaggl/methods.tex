...
gene-by-gene model pools evidence on the log scale,
\begin{equation}
L_{ij}^{\mathrm{multi}} = \sum_t L_{ij}^{(t)},
\label{eq:eaggl_gene_gene_multi_anchor_logbf}
\end{equation}
and then performs a single calibration to $P_{ij}^{\mathrm{same}}$. Thus an anchor with
no evidence contributes zero log-evidence rather than diluting support from another
anchor. If anchor-specific gene relevance probabilities are available, EAGGL gates the
pooled pair matrix by the noisy union of pairwise direct-support gates,
\begin{equation}
G_{ij}^{\mathrm{multi}} =
1 - \prod_t \left(1 - p_i^{(t)}p_j^{(t)}\right).
\label{eq:eaggl_gene_gene_multi_anchor_gate}
\end{equation}
An expert `any' mode instead computes each anchor's calibrated target matrix
$M_{ij}^{(t)}$ and combines them as a noisy-OR union,
\begin{equation}
M_{ij}^{\mathrm{any}} = 1 - \prod_t \left(1 - M_{ij}^{(t)}\right).
\label{eq:eaggl_gene_gene_any_anchor}
\end{equation}
An expert `mean' mode preserves the previous arithmetic-mean target,
\begin{equation}
M_{ij}^{\mathrm{mean}} = \frac{1}{T}\sum_t M_{ij}^{(t)},
\label{eq:eaggl_gene_gene_mean_anchor}
\end{equation}
and is intended for backward-compatible sensitivity analysis. Gene-by-annotation
multi-anchor EAGGL does not require a corresponding mean or consensus mode because its
