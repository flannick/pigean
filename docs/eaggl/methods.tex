...
for each retained gene from the full read annotation matrix before retained-gene-set
filtering,
\begin{equation}
a_i = \log\left(1 + \#\{s:X_{is}\ne 0\}\right).
\end{equation}
For each anchor trait, EAGGL fits a Gamma log-link background model over upper-triangle
gene pairs,
\begin{equation}
\log E_0\{\exp(L_{ij})\}
= h(a_i,a_j),
\end{equation}
where $h$ is a ridge-penalized linear function of an intercept, $a_i+a_j$,
$|a_i-a_j|$, and $a_i a_j$. The corrected evidence is
\begin{equation}
L_{ij}^{\mathrm{corr}} = L_{ij} - h(a_i,a_j).
\end{equation}
...
is opt-in and is intended to remove global annotation-count exposure, not to change the
