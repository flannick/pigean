...
In the implementation, the selected $\phi$, the candidate-level search diagnostics, and the thresholds used for the search are written to the run log and parameter output so that the final reported factorization is auditable after the fact. The candidate diagnostics include the redundancy summaries, factor-mass summaries, and convergence diagnostics such as the final change in ARD weights and whether the scout fit hit the candidate iteration cap. For very large annotation collections, the implementation evaluates candidate $\phi$ values on sentinel subsets of relatively uncorrelated genes and gene sets and may also use a smaller candidate iteration budget, then reruns the final reported factorization on the full retained matrix with the selected $\phi$. The current defaults use sentinel sizes of 1000 representative genes and 1000 representative gene sets and one restart per candidate, with larger restart counts available as an expert stability check. Candidate values are explored locally around the initial $\phi$ using multiplicative steps of two under a bounded total evaluation budget, and once the search crosses into a zero-factor or capped regime it may spend the remaining budget on geometric-midpoint refinements rather than continuing to jump by full decades. This shortcut changes only the cost of the search procedure; it does not change the final fitting target once $\phi$ has been chosen.
