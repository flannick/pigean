...
\item `trait_n_eff` = $\left(\sum_f s_t^{(b)}\right)^2 \big/ \sum_f \left(s_t^{(b)}\right)^2$
\item `retained_n_eff` = $\left(\sum_f s_{t,\mathcal{M}f}^{(b)}\right)^2 \big/ \sum_f \left(s_{t,\mathcal{M}f}^{(b)}\right)^2$
