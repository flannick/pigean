...
When more than one anchor user is present, EAGGL summarizes the support-capture relevance of factor $k$ to any selected anchor by a noisy-OR-style aggregation:
\begin{equation}
r_k^{\mathrm{any}}
=
1 -
...
\end{equation}
This has the intended semantics that $r_k^{\mathrm{any}}$ is a bounded summary of how strongly factor $k$ is captured by at least one selected anchor under the relevance model above. In the single-user case, Eq.~\eqref{eq:eaggl_factor_any_relevance} reduces to the anchor-specific relevance itself.

When anchor phenotypes are present and canonical trait linkage is also computed, EAGGL additionally reports `anchor_joint`, `anchor_marginal`, `anchor_any_joint`, and `anchor_any_marginal` summaries derived from the canonical linkage calculations in Section~\ref{sec:postfactor_revised}. During the transition period, `any_relevance` remains as a compatibility field and coincides with the anchor-any joint support-capture summary in those phenotype-anchored linkage runs.
...
\subsection{Canonical trait linkage by support-preserving thresholded projection}
...
EAGGL then reports two linkage summaries from the same internal matching inputs. The \emph{marginal fraction} of factor $k$ for trait $t$ is the one-factor bounded projection
\begin{equation}
m_{tk}^{(b)}
=
...
\end{equation}
and the \emph{joint fraction} is the all-factor competitive projection
\begin{equation}
j_t^{(b)}
=
...
The coefficient $m_{tk}^{(b)}$ is interpreted as the standalone support fraction of phenotype $t$ captured by factor $k$ when that factor is viewed alone. The coefficient $j_{tk}^{(b)}$ is interpreted as the unique explanatory support fraction of factor $k$ for phenotype $t$ after the other factors compete. In contrast to a projection normalized only on the retained masked slice, Eqs.~\eqref{eq:eaggl_trait_linkage_marginal} and \eqref{eq:eaggl_trait_linkage_joint} preserve the scale of the full thresholded trait while still solving on the retained factor basis. Broad phenotypes can therefore have large total thresholded support $A_t^{(b)}$ without automatically receiving inflated linkage weights when only a small masked subset of their support survives into the factor basis.

For reporting, EAGGL writes:
\begin{itemize}
\item `trait_total_support` = $A_t^{(b)}$
\item `retained_trait_support` = $\sum_f s_{t,\mathcal{M}f}^{(b)}$
\item `retained_fraction` = `retained_trait_support` / `trait_total_support`
\item `joint_fraction` = $j_{tk}^{(b)}$
\item `marginal_fraction` = $m_{tk}^{(b)}$
\item `joint_support_mass` = `trait_total_support` $\times$ `joint_fraction`
\item `marginal_support_mass` = `trait_total_support` $\times$ `marginal_fraction`
\item `factor_total_mass` = $S_k^{(b)}$ in the factor summary output
\end{itemize}

\paragraph{Interpretation.}
Canonical trait linkage is a descriptive quantity. The marginal fractions answer:
\begin{quote}
``How much does factor $k$ link to phenotype $t$ when factor $k$ is viewed alone?''
\end{quote}
The joint fractions answer:
\begin{quote}
...
EAGGL also reports retained-basis diagnostics alongside the linkage scores: total thresholded trait support, retained masked trait support, retained fraction, total thresholded feature count, retained masked feature count, and a low-retention flag for traits with very small retained support.
...
\begin{itemize}
\item \textbf{Canonical trait linkage} gives bounded marginal and joint support-capture fractions describing how well the retained high-confidence phenotype profile is represented by each factor. This is the primary annotation layer for interpreting factors with external phenotypes.
