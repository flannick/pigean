...
- Ordinary fit loss is warning-level in target-size mode. `--learn-phi-fit-loss-warning-frac` marks candidates whose reconstruction loss exceeds the configured threshold relative to the best-fit candidate, but they remain eligible if the structural guardrails are satisfied. `--learn-phi-max-severe-fit-loss-frac` is the separate hard stop for pathological underfit.
