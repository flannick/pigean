...
class EagglCliTest(unittest.TestCase):
    ...
    def test_default_help_includes_phi_learning_controls(self) -> None:
        ...
        self.assertIn("--learn-phi-target-gene-effective-support", proc.stdout)
    ...
    def test_cli_manifest_tiers_cover_recent_factor_and_gene_list_flags(self) -> None:
        ...
        self.assertEqual(metadata["--factor-phi-metrics-out"]["documentation_target"], "advanced_workflows")
    ...
    def test_invalid_gene_gene_settings_return_usage_error(self) -> None:
        ...
        proc = self._run("factor", "--learn-phi", "--learn-phi-target-gene-effective-support", "25", "--learn-phi-max-redundancy-q90", "1.2")
        self.assertEqual(proc.returncode, 2)
        err = (proc.stderr or "") + (proc.stdout or "")
    ...
    def test_invalid_phi_search_shortcut_settings_return_usage_error(self) -> None:
        ...
        proc = self._run("factor", "--learn-phi", "--learn-phi-target-gene-effective-support", "25", "--learn-phi-prune-gene-sets-num", "0")
        self.assertEqual(proc.returncode, 2)
        err = (proc.stderr or "") + (proc.stdout or "")
        self.assertIn("--learn-phi-prune-gene-sets-num must be at least 1", err)
        self.assertNotIn("Traceback", err)

        proc = self._run("factor", "--learn-phi", "--learn-phi-target-gene-effective-support", "25", "--learn-phi-max-num-iterations", "0")
        self.assertEqual(proc.returncode, 2)
        err = (proc.stderr or "") + (proc.stdout or "")
    ...
    def test_projection_only_factor_gene_clusters_requires_requested_projection(self) -> None:
