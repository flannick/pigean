...
class FactorStageHelpersTest(unittest.TestCase):
    ...
    def test_write_factor_outputs_emits_only_requested_targets(self) -> None:
        ...
        options = _options(
            ...
            gene_clusters_full_out="g_cluster_full.tsv",
    ...
    def test_write_full_gene_clusters_includes_filtered_genes_with_projected_values(self) -> None:
        runtime = eaggl.EagglState(background_prior=0.05, batch_size=10)
        runtime.params = {"phi": 0.0, "rel_tol": 1e-4, "run_transpose": True}
        runtime.exp_lambdak = np.array([1.0, 1.0])
        runtime.factor_labels = ["label1", "label2"]
        runtime.exp_gene_set_factors = np.array([[1.0, 0.0], [0.0, 1.0]], dtype=float)
        runtime.exp_gene_factors = np.array([[9.0, 0.0], [0.0, 9.0]], dtype=float)
        ...
        runtime.gene_in_discovery_mask = np.array([True, False], dtype=bool)
        runtime.X_orig = np.array([[1.0, 0.0], [0.0, 1.0]], dtype=float)
        runtime.X_orig_missing_genes = np.array([[0.25, 0.75]], dtype=float)
        runtime.gene_sets = ["gs1", "gs2"]
        runtime.genes = ["retained_a", "retained_b"]
        runtime.genes_missing = ["filtered_c"]
        runtime.combined_prior_Ys = np.array([2.0, 1.0], dtype=float)
        runtime.combined_prior_Ys_missing = np.array([0.5], dtype=float)
        runtime.Y = np.array([1.5, 0.5], dtype=float)
        runtime.Y_missing = np.array([0.25], dtype=float)
        runtime.priors = np.array([0.2, 0.1], dtype=float)
        runtime.priors_missing = [0.05]

        retained_projection = np.array([[0.8, 0.2], [0.1, 0.9]], dtype=float)
        missing_projection = np.array([[0.3, 0.7]], dtype=float)
        with mock.patch.object(
            runtime,
            "_project_H_with_fixed_W",
            side_effect=[retained_projection, missing_projection],
        ):
            with tempfile.TemporaryDirectory() as tmpdir:
        ...
        self.assertIn("filtered_c\t0.5\t0.25\t0.05\tFalse", content)
        self.assertIn("retained_a\t2\t1.5\t0.2\tTrue\tFactor1\tlabel1\t0.8\t0.2\t0.9701\t0.2425", content)
        self.assertIn("retained_b\t1\t0.5\t0.1\tFalse\tFactor2\tlabel2\t0.1\t0.9\t0.1104\t0.9939", content)
        self.assertIn("filtered_c\t0.5\t0.25\t0.05\tFalse\tFactor2\tlabel2\t0.3\t0.7\t0.3939\t0.9191", content)
    ...
    def test_projection_only_pheno_cluster_stage_projects_from_gene_factors(self) -> None:
        ...
        self.assertIsNone(runtime.trait_linkage_beta_tilde)
        self.assertIn("trait\tfactor\tis_anchor\tnnls_loading", content)
        self.assertIn("trait_neff", content)
        self.assertIn("factor_num_genes", content)
        self.assertIn("factor_weight_sum", content)
        self.assertIn("trait_response_source", content)
        self.assertIn("factor_gene_basis", content)
        self.assertIn("factor_loading_threshold", content)
        self.assertNotIn("beta_uncorrected", content.splitlines()[0])
        self.assertNotIn("marginal_lift", content)
    ...
    def test_projection_only_anchor_and_external_trait_linkage_share_normalization_logic(self) -> None:
        ...
        self.assertTrue(result.ran)
        np.testing.assert_array_equal(runtime.trait_linkage_is_anchor, [False, False])
        np.testing.assert_allclose(runtime.factor_anchor_relevance[:, 0], runtime.trait_linkage_joint[0, :], atol=1e-8)
        self.assertEqual(runtime.factor_relevance.shape, (2,))
        self.assertIsNone(runtime.trait_linkage_marginal)
        np.testing.assert_allclose(runtime.factor_anchor_marginal_relevance, runtime.factor_anchor_relevance, atol=1e-8)
        np.testing.assert_allclose(runtime.factor_marginal_relevance, runtime.factor_relevance, atol=1e-8)
    ...
    def test_write_matrix_factors_reports_factor_total_mass(self) -> None:
        ...
        self.assertIn("lambda\tfactor_tier\tcombined_mass_fraction", content.splitlines()[0])
        self.assertNotIn("any_relevance", content.splitlines()[0])
        ...
        self.assertIn("Factor1\timmune\t1\tfiltered\t0\t3\t2\t0.75\t1\t0", content)
        self.assertIn("Factor2\tmetabolic\t2\tfiltered\t0\t7\t600\t0.005\t0.04\t1", content)
    ...
    def test_write_matrix_factors_uses_clean_anchor_summary_columns(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            runtime.write_matrix_factors(str(factors_path), factor_output_scope="all")
            runtime.write_matrix_factors(str(factors_anchor_path), write_anchor_specific=True, factor_output_scope="all")
